...
func void B_Preach_Vatras(var int satz)
{
	...
	if(satz == 6)
	{
		AI_Output(self,self,"DIA_Vatras_PREACH_05_06");	//Adonos byl velmi spokojen s tím, co se přihodilo. A jeho radost se stejnou měrou týkala všech bytostí bez výjimky.
	};
	if(satz == 7)
	{
	...
	if(satz == 12)
	{
		AI_Output(self,self,"DIA_Vatras_PREACH_05_12");	//Ale s ním byly smeteny i stromy a zvířata. A Adanose obklopil hluboký smutek.
	};
	if(satz == 13)
	{
	...
	if(satz == 14)
	{
		AI_Output(self,self,"DIA_Vatras_PREACH_05_14");	//Ale člověk a bestie vedli válku v Adanosově zemi a stihl je božský hněv.
	};
	if(satz == 15)
	{
	...
	if(satz == 19)
	{
		AI_Output(self,self,"DIA_Vatras_PREACH_05_19");	//A tak vyzval Innose, aby přenechal část své moci Adanosově království, aby ji mohl opět propůjčit člověku pro případ, že by se bestie vrátila.
	};
	if(satz == 20)
	{
