...
instance PC_PRAYSHRINE_UPGRATEBELIARSWEAPON_LVL(C_Info)
{
	npc = PC_Hero;
	nr = 3;
	condition = pc_prayshrine_upgratebeliarsweapon_lvl_condition;
	information = pc_prayshrine_upgratebeliarsweapon_lvl_info;
	permanent = TRUE;
	description = "Beliarův dráp, na tohle ještě nejsem dost silný (potřebuješ mít nejméně Úr 30)";
};


func int pc_prayshrine_upgratebeliarsweapon_lvl_condition()
{
	if((PLAYER_MOBSI_PRODUCTION == MOBSI_PRAYIDOL) && (C_ScHasBeliarsWeapon() == TRUE))
	{
		if(hero.level < 30)
		{
			return TRUE;
...
func void pc_prayshrine_upgratebeliarsweapon_lvl_info()
{
};
