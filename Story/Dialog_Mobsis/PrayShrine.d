...
var int shrineisobsessed_nw_city_merchant_temple_in;
...
func void PC_PrayShrine_Pray_Info()
{
	...
	else
	{
		...
		Info_AddChoice(PC_PrayShrine_Pray,"Chci se pomodlit a nic nevěnovat.",PC_PrayShrine_Pray_NoPay);
...
instance PC_PRAYSHRINE_BLESSARMOR_FINAL(C_Info)
{
	npc = PC_Hero;
	nr = 2;
	condition = pc_prayshrine_blessarmor_final_condition;
	information = pc_prayshrine_blessarmor_final_info;
	permanent = TRUE;
	description = BLESS_ARMOR;
};


func int pc_prayshrine_blessarmor_final_condition()
{
	if((PLAYER_MOBSI_PRODUCTION == MOBSI_PrayShrine) && (hero.guild == GIL_PAL) && (Npc_GetDistToWP(hero,"NW_MONASTERY_CHAPELL_02") <= 500) && (Npc_HasItems(hero,itwr_armorbless2) >= 1) && ((Npc_HasItems(hero,ItAr_PAl_H) >= 1) || (Npc_HasItems(hero,itar_pal_helm2) >= 1)))
	{
		return TRUE;
	};
};

func void pc_prayshrine_blessarmor_final_info()
{
	if(ShrineIsObsessed == TRUE)
	{
		SC_IsObsessed = TRUE;
	...
	else if((Npc_HasItems(hero,ItMi_HolyWater) >= 1) || (Npc_HasItems(hero,itfo_holywater) >= 1))
	{
		Npc_RemoveInvItems(hero,ItMi_HolyWater,1);
		Npc_RemoveInvItems(hero,itfo_holywater,1);
		if(Npc_HasItems(hero,itar_pal_helm2) >= 1)
		{
			Npc_RemoveInvItems(hero,itar_pal_helm2,1);
			CreateInvItems(hero,itar_palinnos2,1);
			Wld_PlayEffect("spellFX_PalHeal_ORIGIN",hero,hero,0,0,0,FALSE);
			Snd_Play("MFX_Heal_Cast");
			B_GivePlayerXP(XP_SwordBlessed2);
		}
		else
		{
			...
			CreateInvItems(hero,itar_palinnos,1);
			Wld_PlayEffect("spellFX_PalHeal_ORIGIN",hero,hero,0,0,0,FALSE);
			Snd_Play("MFX_Heal_Cast");
			B_GivePlayerXP(XP_SwordBlessed2);
		};
	...
	{
		AI_PrintScreen(PRINT_NOINNOSTEARS2,-1,YPOS_GoldGiven,FONT_ScreenSmall,2);
	};
};
