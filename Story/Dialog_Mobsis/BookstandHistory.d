...
func void Use_BookstandHistory2_S1()
{
	...
	if(Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her))
	{
		...
		Doc_PrintLines(nDocID,0,"Tento ostrov je proslulý zejména tím, že se na něm nachází slavné Hornícké údolí. Jeho pověst je pochybná, neboť je po mnoho let obklopovala neprostupná magická bariéra, a proto sloužilo jako věznice pro trestance z celé říše.");
		Doc_PrintLines(nDocID,0,"Ti zde dolovali magickou rudu, jejíž ložiska se ukrývala hluboko pod povrchem země. ");
		Doc_SetMargins(nDocID,-1,30,20,275,20,1);
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLines(nDocID,1,"Půda za hranicemi Khorinisu je velmi úrodná, proto tu najdete mnoho selských usedlostí, kde se pěstují zejména tuříny a chovají ovce.");
		Doc_PrintLine(nDocID,1,"");
