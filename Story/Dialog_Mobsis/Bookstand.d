...
func void Use_FINALDRAGONEQUIPMENT_S1()
{
	...
	if(Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her))
	{
		...
		Doc_SetMargins(nDocID,0,275,20,30,20,1);
		Doc_PrintLine(nDocID,0,"");
		Doc_PrintLines(nDocID,0,"...Doufám, že kupole ochrání rudu před zlobou Beliarovou. Král je ve své naivitě schopen věřit, že jsme ji postavili proto, aby žádný z vězňů neunikl. No, jestli tahle lest poslouží vyšším cílům, ať se tak stane. Mohu jen doufat, že nám ještě zbývá dost času pro přípravy na boj. Jakmile kupole obklopí Hornické údolí, vynaložím všechnu svou moc, aby se boj už více nezdržoval. ");
		Doc_PrintLines(nDocID,0,"");
		if(hero.guild == GIL_KDF)
		{
			PlayerGetsAmulettOfDeath = TRUE;
			PLAYER_TALENT_RUNES[SPL_MasterOfDisaster] = TRUE;
			B_LogEntry(TOPIC_TalentRunes,"K přípravě runy 'Božský zásah' je třeba: 1 svěcená voda, žádný svitek s kouzlem");
			Doc_SetMargins(nDocID,-1,30,20,275,20,1);
			Doc_PrintLine(nDocID,1,"");
		...
		else if(hero.guild == GIL_PAL)
		{
			...
			B_LogEntry(TOPIC_TalentRunes,"Přísady pro runu 'tajného teleportu' jsou: 1 Innosova slza");
			Doc_SetMargins(nDocID,-1,30,20,275,20,1);
			Doc_PrintLine(nDocID,1,"");
			...
			Doc_PrintLine(nDocID,1,"");
			Doc_PrintLines(nDocID,1,"Teď už jsem si skoro jist, že právě Innosovy slzy používali za dávných časů paladinové při dnes již zapomenutím rituálu svěcení meče. Takže tahle malá lahvička, co jsem našel, by měla posvěcené zbrani přidat ještě další sílu.");
			Doc_PrintLine(nDocID,1,"");
			Doc_Show(nDocID);
		}
		else
		{
			...
			Doc_PrintLines(nDocID,1,"Chceš-li zbroj z dračích šupin co možná vytvrdit, musíš je potáhnout rudou, která se těží v údolí na ostrově Karynis.");
			Doc_PrintLine(nDocID,1,"");
