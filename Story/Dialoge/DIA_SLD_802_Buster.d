...
func void DIA_Buster_Hello_ImNoDanger()
{
	...
	AI_Output(self,other,"DIA_Buster_Hello_ImNoDanger_13_02");	//V jednom kuse tady bojujeme se skřety. Nebo strážemi z města. Teda, jsou to pěkně neodbytní prevíti, to teda jo. (zasměje se)
	Info_ClearChoices(DIA_Buster_Hello);
...
func void DIA_Buster_WannaJoin_Info()
{
	...
	if((self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_WON) || (self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_LOST))
	{
		...
		B_LogEntry(TOPIC_SLDRespekt,"Když jsem Bustera porazil, nebude mi už bránit v cestě žoldnéřských řad.");
	}
	else
	{
...
func void DIA_Buster_BringTrophyShadowbeast_Info()
{
	...
	else
	{
		...
		if(BusterTrophyShadowbeastCount == 1)
		{
			...
			Npc_RemoveInvItems(self,ItAt_ShadowHorn,50);
