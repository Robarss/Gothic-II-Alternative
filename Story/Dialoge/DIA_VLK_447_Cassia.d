...
func void DIA_Cassia_Aufnahme_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Aufnahme_16_02");	//Vezmi si tenhle klíč a něco na sebe. Je od dveří od hotelu. (úsměv) Tak nebudeš muset pokaždé plavat.
	B_GiveInvItems(self,other,ItKe_ThiefGuildKey_Hotel_MIS,1);
	CreateInvItems(other,itar_noble_thief,1);
	AI_PrintScreen("Cassia ti daruje zlodějský kabátec.",-1,YPOS_ItemTaken,FONT_ScreenSmall,2);
	...
	B_LogEntry(Topic_Diebesgilde,"Byj jsem přijat do zlodějského cechu.");
...
func void DIA_Cassia_abgeben_Info()
{
	...
	if(B_GiveInvItems(other,self,ItMi_BloodCup_MIS,6))
	{
		...
		AI_Output(self,other,"DIA_Cassia_abgeben_16_03");	//Můžeš hned vzít svou odměnu. Dík, žes to pro mě udělal.
...
func void dia_cassia_akrobati_info()
{
	...
	if((Npc_HasItems(other,ItMi_Gold) > 500) && (other.attribute[ATR_DEXTERITY] > 59) && (hero.lp >= 10))
	{
		...
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_01");	//Brzy získáš kontrolu nad svým tělem a budeš schopen skákat mnohem dál.
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_02");	//Naučil ses, jak se stočit a jak nejlépe při pádu dopadnout na zem. Ale nezapomeň - nejsi nesmrtelný!.
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_04");	//Akrobacie může být také velmi užitečná v boji. Můžeš velmi rychle měnit vzdálenost mezi tebou a tvým nepřítelem. Hodně štěstí!.
		PrintScreen("",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
		PrintScreen("Přijd až budeš připraven!",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
