...
func void DIA_Lee_PMSchulden_Info()
{
	...
	if(B_GetTotalPetzCounter(self) > Lee_LastPetzCounter)
	{
		...
		{
			AI_Output(self,other,"DIA_Lee_PMSchulden_04_03");	//Bude tedy po tobě chtít také víc peněz.
...
func void DIA_Lee_PETZMASTER_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Lee_PETZMASTER_04_00");	//Kýho čerta - (překvapeně) - to jsi TY ten nováček, který jde z jednoho maléru do druhého?
...
instance DIA_Lee_Paladine(C_Info)
{
	...
	permanent = FALSE;
	description = "Potřebuju si nutně promluvit s paladiny z města.";
};
...
func void DIA_Addon_Lee_Ranger_Info()
{
	...
	AI_Output(other,self,"DIA_Addon_Lee_Ranger_15_02");	//Ale no tak, řekni mi o tom.
	AI_Output(self,other,"DIA_Addon_Lee_Ranger_04_03");	//Nejsem do toho přímo zapletený. Vím jen, že existuje tahle tajná frakce, za kterou jsou mágové Vody.
	AI_Output(self,other,"DIA_Addon_Lee_Ranger_04_04");	//Od pádu bariéry, už nejsem vázaný dohodou, kterou jsem uzavřel s mágy Vody.
	AI_Output(self,other,"DIA_Addon_Lee_Ranger_04_05");	//Samozrejmě, že se ti pokusím pomoct, když budeš něco potřebovat. Avšak, mám málo času poslední dobou a své problémy, které mě zaneprázdňují.
	AI_Output(self,other,"DIA_Addon_Lee_Ranger_04_06");	//Mohl by sis promluvit s Cordem pokud o tom chceš získat nějaké informace. Pokud vím, tak je jedním z nich.
...
func void DIA_Lee_JoinNOW_Info()
{
	...
	if((Lee_StimmenOK == TRUE) && (Lee_OnarOK == FALSE))
	{
		...
		else
		{
			...
			AI_Output(self,other,"DIA_Lee_JoinNOW_04_17");	//Má to co do činění s paladiny. Je na čase, aby ses za nimi vypravil.
...
instance DIA_Lee_AngebotSuccess(C_Info)
{
	...
	permanent = FALSE;
	description = "Donesl jsem lordu Hagenovi tvoji nabídku příměří.";
};
...
func void DIA_Lee_AngebotSuccess_Info()
{
	...
	AI_Output(other,self,"DIA_Lee_AngebotSuccess_15_02");	//Řekl, že tobě může zajistit prominutí tvých činů, ale ne tvým lidem.
	...
	AI_Output(self,other,"DIA_Lee_AngebotSuccess_04_06");	//Že bych sám vytáhl krk z oprátky a opustil své muže, to vůbec nepřipadá v úvahu.
...
func void DIA_Lee_ArmorM_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Lee_ArmorM_04_03");	//Torlof dostal od Onara úkol, o který se měl již před časem postarat.
		AI_Output(self,other,"DIA_Lee_ArmorM_04_04");	//Takže to nejdřív zařiď - pak si můžeme promluvil o lepším brnění!
	};
};
...
instance DIA_Lee_BuyArmorM(C_Info)
{
	...
	permanent = TRUE;
	description = "Koupit si středně těžkou žoldnéřskou zbroj. Ochrana: zbraně 50, šípy 50, cena 1000 zlaťáků.";
};
...
instance DIA_Lee_BuyArmorH(C_Info)
{
	...
	permanent = TRUE;
	description = "Koupit těžkou žoldnéřskou zbroj. Ochrana: zbraně 80, šípy 80, cena: 2500 zlaťáků.";
};
...
func void DIA_Lee_Richter_Info()
{
	...
	B_LogEntry(TOPIC_RichterLakai,"Lee chce, abych našel nějaký inkriminující důkaz proti khorinisskému soudci. Mám mu proto nabídnout své služby a přitom mít oči na stopkách.");
...
func void DIA_Lee_RichterBeweise_Info()
{
	...
	AI_Output(other,self,"DIA_Lee_RichterBeweise_15_02");	//Najal si pár ranařů, aby přepadli místodržícího Khorinisu.
...
func void DIA_Lee_TalkAboutBennet_Info()
{
	...
	AI_Output(self,other,"DIA_Lee_TalkAboutBennet_04_01");	//Takže jsi už o tom slyšel. Ti bastardi ho zavřeli, jen tak pro nic za nic.
	if((hero.guild != GIL_MIL) && (hero.guild != GIL_PAL))
	{
...
func void DIA_Lee_DoAboutBennet_Info()
{
	...
	AI_Output(self,other,"DIA_Lee_DoAboutBennet_04_01");	//Ještě nevím. Pár chlapců by rádo vtrhlo do města povyráželi lordu Hagenovi všechny zuby, jeden po druhém.
	...
	B_LogEntry(TOPIC_RescueBennet,"Když dostatečně rychle neprokážu Bennetovu nevinu, Lee za další následky už neponese zodpovědnost. Jeho lidé bez váhání kdykoliv napadnou město a Benneta se zbaví násilím.");
	if(!Npc_IsDead(Lares))
	{
	...
	{
		AI_Output(self,other,"DIA_Lee_DoAboutBennet_04_07");	//Ano, a abych nezapomněl... Buster s tebou chce mluvit. Nechtěl mi říct, o co jde. Možná by ses měl za ním stavit!
	};
};
...
func void DIA_Lee_CanHelpYou_Info()
{
	...
	AI_Output(self,other,"DIA_Lee_CanHelpYou_04_01");	//Jasně, troška inteligence a rozvahy nemůže být na škodu.
...
instance DIA_Lee_DRACHENEI(C_Info)
{
	...
	description = "Ještěrani roznášejí dračí vejce po celé zemi.";
};
...
{
	AI_Output(other,self,"DIA_Lee_DRACHENEI_15_00");	//Ještěrani roznášejí dračí vejce po celé zemi.
	B_GivePlayerXP(XP_Ambient);
	AI_Output(self,other,"DIA_Lee_DRACHENEI_04_01");	//Mělo mě to napadnout. Je na čase vypadnout.
	if(hero.guild == GIL_DJG)
	{
		...
		AI_Output(self,other,"DIA_Lee_DRACHENEI_04_04");	//Ze skořápek by možná šlo dělat brnění nebo tak něco.
...
func void DIA_Lee_KAP4_Perm_Info()
{
	...
	AI_Output(self,other,"DIA_Lee_KAP4_Perm_04_01");	//No, co odešel Sylvio, tady byl konečně docela klid.
	AI_Output(other,self,"DIA_Lee_KAP4_Perm_15_02");	//To nezní špatně.
	AI_Output(self,other,"DIA_Lee_KAP4_Perm_04_03");	//Práce bohužel neubylo. Muži jsou čím dál tím nespokojenější, protože musejí pracovat i za Sylviovy muže.
...
func void DIA_Lee_GetShip_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Lee_GetShip_04_04");	//Konec konců máme soudce ve své moci. Zkus za ním jít a vymáčknout z něj dopis, který by tě k tomu opravňoval.
	...
	{
		AI_Output(self,other,"DIA_Lee_GetShip_04_05");	//Mám tady falešné oprávnění. S jeho pomocí tě nechají stráže projít.
...
func void DIA_Lee_GetShip_crew()
{
	...
	B_LogEntry(Topic_Crew,"Ve věci posádky pro mě toho Lee moc udělat nemohl. Ale poradil mi, že bych měl najmout jen lidi, kterým mohu věřit. Mám se zeptat ještě Benneta, mohlo by ho to zajímat.");
};
...
instance DIA_Lee_GotRichtersPermissionForShip(C_Info)
{
	...
	description = "To povolení zafungovalo. Loď je teď moje...";
};
