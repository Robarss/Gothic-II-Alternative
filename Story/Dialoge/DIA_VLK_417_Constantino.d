...
func void DIA_Constantino_HerbsRunning_Success()
{
	...
	AI_Output(self,other,"DIA_Constantino_HerbsRunning_Success_10_02");	//Při Adanosovi! Všechno je opravdu v pořádku.
	...
	Log_CreateTopic(TOPIC_Lehrling,LOG_MISSION);
	Log_SetTopicStatus(TOPIC_Lehrling,LOG_Running);
	B_LogEntry(TOPIC_Lehrling,"Constantino mě příjme do učení, jen když získám doporučení od čtyřech mistrů.");
	Info_ClearChoices(DIA_Constantino_HerbsRunning);
};
...
func void DIA_Constantino_LEHRLING_Yes()
{
	...
	CreateInvItems(other,ITAR_Barkeeper,1);
	AI_PrintScreen("Oděv od Constantina získán.",-1,YPOS_ItemTaken,FONT_ScreenSmall,2);
...
func void DIA_Constantino_Alchemy_Info()
{
	...
	AI_Output(self,other,"DIA_Constantino_Alchemy_10_04");	//OK přípravě lektvaru s pomocí alchymistické kolony potřebuješ laboratorní baňku.
	...
	B_LogEntry(TOPIC_CityTeacher,"Constantino naučí alchymistickému umění.");
};
...
func void DIA_Constantino_TEACH_Health02()
{
	...
	{
		AI_Output(self,other,"DIA_Constantino_TEACH_Health02_10_00");	//OK přípravě extraktu hojivé síly si musíš opatřit léčivé byliny a luční pohanku.
...
func void DIA_Constantino_TEACH_Mana02()
{
	if(B_TeachPlayerTalentAlchemy(self,other,POTION_Mana_02))
	{
		...
		AI_Output(self,other,"DIA_Constantino_TEACH_Mana02_10_01");	//Při přípravě tohoto elixíru stačí mít trošku citu a nechat jej jemně přejít varem. Jeho ingredience tvoří ohnivé kopřivy a luční pohanka.
	};
	Info_ClearChoices(DIA_Constantino_TEACH);
};
