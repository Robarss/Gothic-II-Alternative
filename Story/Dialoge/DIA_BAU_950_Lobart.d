...
func void DIA_Lobart_STOLENCLOTHS_DontHaveIt()
{
	...
	AI_Output(self,other,"DIA_Lobart_STOLENCLOTHS_DontHaveIt_05_01");	//Prodals je, co? (řve) Já ti ukážu, mladíku!
...
func void DIA_Lobart_BuyClothes_Info()
{
	...
	Info_AddChoice(DIA_Lobart_BuyClothes,"Tak mi tedy dej ten pracovní oděv (ochrana: zbraně 15, šípy 15).",DIA_Lobart_BuyClothes_BUY);
};
...
instance DIA_Lobart_OnarStory(C_Info)
{
	...
	permanent = FALSE;
	description = "Řekni mi něco o tom Onarovi.";
};
...
func void DIA_Lobart_RuebenRunning_Info()
{
	...
	if(Npc_HasItems(other,ItPl_Beet) >= 20)
	{
		...
		if(hero.guild == GIL_NONE)
		{
			Info_AddChoice(DIA_Lobart_RuebenRunning,"Udělej mi za ty hadry lepší cenu!",DIA_Lobart_RuebenRunning_Billiger);
		};
...
{
	AI_Output(other,self,"DIA_Lobart_RuebenRunning_Billiger_15_00");	//Udělej mi za ty hadry lepší cenu!
...
instance DIA_Lobart_ANDREHELPLOBART(C_Info)
{
	...
	description = "Andre mě poslal za tebou. Máš pro mě nějakou práci?";
};
...
func void DIA_Lobart_DMT_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Lobart_DMT_05_04");	//Jenom zase nějaká choroba. Už moc dlouho jen leží v posteli. Stává se jí to často.
	};
