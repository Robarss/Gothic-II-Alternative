...
func void DIA_Pyrokar_Auge_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Pyrokar_ALL_11_05");	//Dobrá, už tě nebudeme dál zdržovat od tvé práce - můžeš jít.
		AI_StopProcessInfos(self);
	};
};
...
instance DIA_Addon_Pyrokar_MissingPeople(C_Info)
{
	...
	description = "Někteří občané Khorinisu zmizeli za podivných okolností.";
};
...
{
	AI_Output(other,self,"DIA_Addon_Pyrokar_MissingPeople_15_00");	//Někteří občané Khorinisu zmizeli za podivných okolností.
	AI_Output(self,other,"DIA_Addon_Pyrokar_MissingPeople_11_01");	//Jsem si toho vědom a je to politováníhodné. Nicméně jsme souhlasili s mágy Vody, že na tu věc dohlídnou.
	AI_Output(self,other,"DIA_Addon_Pyrokar_MissingPeople_11_02");	//To samé se týka té záležistosti s těmi podivnými zemětřesenímy v severovýchodní oblasti Khorinosu.
	AI_Output(self,other,"DIA_Addon_Pyrokar_MissingPeople_11_03");	//V tomto okamžiku nic nenasvědčuje tomu, že naše zakročení by mělo zaručený výsledek. Z tohoto důvodu se nebudeme do ničeho pouštět.
	...
	B_LogEntry(TOPIC_Addon_WhoStolePeople,"Mágové Ohně litují zmizelých občanů Khorinosu . Nicméně říkají, že by to byl úkol pro mágy Vody. Neočekávejte tedy pomoc od kláštera ohnivých mágů.");
...
func void DIA_Pyrokar_FIRE_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_FIRE_11_07");	//V tom případě - ať je po tvém. Až budeš připraven, dostaneš od každého mága Rady úkol, jejž musíš splnit.
...
{
	if((MIS_SCHNITZELJAGD == LOG_Running) && Npc_IsInState(self,ZS_Talk) && (other.guild == GIL_NOV) && Mob_HasItems("MAGICCHEST",ItMi_FireRuneBlank))
	{
		return TRUE;
	};
};
...
{
	if((MIS_SCHNITZELJAGD == LOG_Running) && (hero.guild == GIL_NOV) && !Mob_HasItems("MAGICCHEST",ItMi_FireRuneBlank) && ((Npc_HasItems(other,ItMi_FireRuneBlank) >= 1) || (Npc_HasItems(other,ItRu_FireBolt) >= 1)))
	{
		return TRUE;
	};
};
...
func void DIA_Pyrokar_Todo_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Pyrokar_Todo_11_03");	//... ještě musíš dokončit Serpentesův test.
	};
	AI_StopProcessInfos(self);
};
...
instance DIA_Pyrokar_OATH(C_Info)
{
	...
	permanent = FALSE;
	description = "Jsem připraven vstoupit to Kruhu ohně.";
};
...
{
	AI_Output(other,self,"DIA_Pyrokar_OATH_15_00");	//Jsem připraven vstoupit to Kruhu ohně.
	AI_Output(self,other,"DIA_Pyrokar_OATH_11_01");	//Dobrá, tak slož svatou Přísahu ohně.
	...
	AI_Output(self,other,"DIA_Pyrokar_OATH_11_09");	//Taky bychom velmi rádi věděli, jak on hodnotí situaci. Takže je ti dovoleno odejít do Khorinisu.
...
instance DIA_Pyrokar_Lernen(C_Info)
{
	...
	permanent = FALSE;
	description = "Co všechno se teď můžu naučit?";
};
...
func void DIA_Pyrokar_Lernen_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_Lernen_11_03");	//Od bratrů v klášteře se můžeš naučit magickým formulím. Každý z nich se zaměřuje na určitou oblast, o které tě mohou hodně naučit.
	...
	B_LogEntry(Topic_KlosterTeacher,"Bratr Parlan mě uvede do prvních magických kruhů.");
...
instance DIA_Pyrokar_Wunsch(C_Info)
{
	...
	permanent = FALSE;
	description = "Chtěl bych vyslovit jedno přání.";
};
...
func void DIA_Pyrokar_TEACH_Info()
{
	...
	if(MIS_SCKnowsWayToIrdorath == TRUE)
	{
		if(B_TeachMagicCircle(self,other,6))
		{
			...
			AI_Output(self,other,"DIA_Pyrokar_TEACH_11_04");	//Teď se ode mě můžeš naučit magické formule posledního kruhu. Pokud po tom tedy toužíš.
			...
			AI_Output(other,self,"DIA_Pyrokar_TEACH_15_07");	//Ano, ušetřil jsi mi tak spoustu těch soudních keců.
...
func void DIA_Pyrokar_BACKFROMOW_Info()
{
	...
	if(hero.guild == GIL_KDF)
	{
		...
		B_LogEntry(TOPIC_DEMENTOREN,"Pyrokar mi vyprávěl o Pátračích, mužích v černých hábitech, kteří jsou Beliarovými nohsledy. Varoval mě, že dokáží ovládat a posednout druhé. Mám se vrátit přímo do kláštera, aby se mi to náhodou nestalo také.");
	};
...
func void DIA_Pyrokar_FOUNDINNOSEYE_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_FOUNDINNOSEYE_11_05");	//Ani ve svých nejhorších nočních můrách jsem si nikdy nepředstavoval, že se zmocní takového druhu moci.
...
func void DIA_Pyrokar_FOUNDINNOSEYE_was_vatras()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_FOUNDINNOSEYE_was_vatras_11_01");	//Privilegium nosit róbu ti ještě neopravňuje k tomu, aby ses vyptával na důvody mých instrukcí, bratře.
	AI_Output(self,other,"DIA_Pyrokar_FOUNDINNOSEYE_was_vatras_11_02");	//Vatras je Adanosův služebník. Vědomosti toho vodního mága nám přinesou trochu světla do tohoto temného času.
...
func void DIA_Pyrokar_FOUNDINNOSEYE_sonnenkreis()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_FOUNDINNOSEYE_sonnenkreis_11_01");	//Každoročně se v čase slunovratu všichni mágové a novicové kláštera vydávají na cestu na toto místo, aby tak přivítali počátek nového cyklu.
...
instance DIA_Pyrokar_XARDASVERTRAUEN(C_Info)
{
	...
	description = "Bez tebe to nepůjde. Vatras není schopen ten rituál jinak připravit.";
};
...
instance DIA_Pyrokar_AUGEGEHEILT(C_Info)
{
	...
	description = "Dokázal jsi to. Innosovo oko je zase v pořádku.";
};
...
{
	AI_Output(other,self,"DIA_Pyrokar_AUGEGEHEILT_15_00");	//Dokázal jsi to. Innosovo oko je zase v pořádku.
	...
	if(hero.guild == GIL_KDF)
	{
		...
		AI_Output(self,other,"DIA_Pyrokar_AUGEGEHEILT_11_04");	//Od tohoto dne jsi členem Rady a budeš nás řád reprezentovat po celém světe. Teď jsi jedním z vyšších ohnivých mágů.
...
instance DIA_Pyrokar_KAP3_READY(C_Info)
{
	...
	description = "Co tu ještě musím učinit?";
};
...
func void DIA_Pyrokar_KAP3_READY_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_KAP3_READY_11_06");	//Budeš k tomu potřebovat esenci z dračího srdce, kterou můžeš s Okem spojit za pomoci alchymistické kolony.
...
func void DIA_Pyrokar_BUCHDERBESSENEN_Info()
{
	...
	if(hero.guild == GIL_KDF)
	{
		...
		AI_Output(self,other,"DIA_Pyrokar_BUCHDERBESSENEN_11_09");	//Najdi ty oběti a přines mi jejich knihy. Měl bych je zprovodit ze světa.
...
func void DIA_Pyrokar_DERMEISTER_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_DERMEISTER_11_03");	//Innos s námi. Temný chrám znovu nabyl svou sílu a teď vysílá své posluhovače do našeho světa.
};
...
func void DIA_Pyrokar_WASISTIRDORATH_Info()
{
	...
	B_LogEntry(TOPIC_BuchHallenVonIrdorath,"Podle Pyrokara jsou Irdoratské síně ztraceným chrámem, kde uctívači vzývali boha Beliara. Musím jej najít.");
};
...
func void DIA_Pyrokar_BUCHIRDORATH_Info()
{
	...
	B_LogEntry(TOPIC_BuchHallenVonIrdorath,"Xardasova kniha Irdoratské síně je ukryta ve sklepeních kláštera, kde ji střeží Talamon. Mágové ji ale nedokáží otevřít - její magickou pečeť prý má na svědomí sám Xardas.");
...
func void DIA_Pyrokar_IRDORATHBOOKOPEN_glueck()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_IRDORATHBOOKOPEN_glueck_11_02");	//Mně se to vůbec nezdařilo a pak je to pro někoho jako ty úplná hračka...
...
instance DIA_Pyrokar_SCWILLJORGEN(C_Info)
{
	...
	description = "Jorgen je zkušeným námořníkem. Mohl by mi pomoci dostat se na ostrov Irdorath.";
};
...
func void DIA_Pyrokar_MACHDTFREI_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_MACHDTFREI_11_01");	//Mmh. Jasně, že je tu jedna věc, kterou bys mohl udělat. Mohly by ti s tím pomoct tvoje dobré vztahy s Xardasem.
...
func void DIA_Pyrokar_AmulettofDeath_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_AmulettofDeath_11_01");	//Innosova aura je amulet, který je povoleno nosit jen největším mágům své doby.
...
func void DIA_Pyrokar_PotionofDeath_Potion()
{
	...
	Info_AddChoice(DIA_Pyrokar_PotionofDeath,"Co myslíš tím 'zbraň'?",DIA_Pyrokar_PotionofDeath_Weapon);
};
...
{
	AI_Output(other,self,"DIA_Pyrokar_PotionofDeath_Weapon_15_00");	//Co myslíš tím "zbraň"?
...
instance DIA_PYROKAR_ARMORBL(C_Info)
{
	npc = KDF_500_Pyrokar;
	nr = 2;
	condition = dia_pyrokar_armorbl_condition;
	information = dia_pyrokar_armorbl_info;
	description = "Našel jsem tento pergamen ale neumím ho přečíst.";
};


func int dia_pyrokar_armorbl_condition()
{
	if((Kapitel == 5) && (Npc_HasItems(hero,itwr_armorbless) >= 1))
	{
		return TRUE;
	};
};

func void dia_pyrokar_armorbl_info()
{
	AI_Output(other,self,"DIA_Pyrokar_ARMORBL_15_00");	//Našel jsem tento pergamen ale neumím ho přečíst.
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_15_01");	//Neuvěřitelné jaké máš štěstí, tohle je prastará modlidba na posvěcení zbroje.
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_11_02");	//Je to napsané ve starem textu a shodou náhod jsem jeden s posledních na ostrově kdo to přečte.
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_11_03");	//Jinak bys musel cestovat až na pevninu do klášteru v nordmaru.
	AI_Output(other,self,"DIA_Pyrokar_ARMORBL_15_04");	//Takže můžu posvětit i zbroj, a jak to udělám?
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_11_05");	//Přeložím ti ten text a podle něj se pomodli u svatyně.
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_11_06");	//Hlavně nezapomeň polívat při modlitbě svěcennou vodou paladinskou zbroj.
	AI_Output(self,other,"DIA_Pyrokar_ARMORBL_11_07");	//Takové pocty se dostalo málo paladinům, a tímto požehnání innose se staneš nejmocnějším paladinem kterého znám, hodně štěstí.
	Npc_RemoveInvItems(hero,itwr_armorbless,1);
	CreateInvItems(hero,itwr_armorbless2,1);
};
