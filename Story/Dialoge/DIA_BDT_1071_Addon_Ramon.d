...
func void DIA_Addon_Ramon_FirstWarn_Info()
{
	if(Npc_IsDead(Franco))
	{
		...
		AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_01");	//Ztřískal jsem Franca.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_02");	//Tak? Dobrá, nyní je to TVŮJ úkol ujistit se, že ti hloupí osli dělají svou práci každý den.
		AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_03");	//Hej, nechci marnit svůj drahocený čas s lovci. Chci do tábora.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_04");	//Tak jo, tys zabil Franca. Tím jsi ze sebe udělal vůdce. Tak dělej svou práci.
		AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_05");	//Dobře, když jsem vůdce, tak si mohu vybrat další osobu, která vstoupí do tábora.
		AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_06");	//A tak nařizuji, že 'JÁ' jsem ta další.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_07");	//(usměvavě) Dobře, nebylo to tak těžké. Nepotřebujeme žádnou další osobu, tak se žádná další osoba přijímat nebude.
		AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_08");	//Thorus by mohl poslat nového vůdce lovcům, tak ten by uvolnil místo mě.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_09");	//(usměvavě) Nevzdáváš se, co? Dobrá, pak se dostaneš do tábora.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_10");	//Thorus se postará o nového vůdce a pošle nám ho.
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_11");	//Tak na co čekáš? A nezapomeň si krumpáč.
	...
	{
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_12");	//Hej, kdo jsi? Nepamatuji si, že by někdo jako ty měl povolení vstoupit.
		if(Npc_KnowsInfo(other,DIA_Addon_Franco_HI))
		{
			AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_15_13");	//Franco mě poslal. Chce vědět, zda-li nepotřebujete nové pracovníky.
		};
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_14");	//Á, právě jednoho potřebujeme. Právě včera jeden z chlápků zdechl.
		...
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_18");	//Tak, řekni Francovi, že potřebujeme nové pracovníky. Chápeš?
	...
	{
		AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_07_19");	//Stůj! Nemůžeš jít dovnitř - tak vypadni!
		AI_StopProcessInfos(self);
	};
...
func void DIA_Addon_Ramon_FirstWarn_YES()
{
	...
	AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_YES_07_01");	//Ano. THORUS. Ten, kdo se ptal po nových dělnících.
	...
	AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_YES_07_03");	//Nechceš mi nyní říct, že už ho znáš už z dřívějška?
	AI_Output(other,self,"DIA_Addon_Ramon_FirstWarn_YES_15_04");	//Dobře ... Ano ...
...
func void DIA_Addon_Ramon_FirstWarn_NO()
{
	...
	AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_NO_07_01");	//Ale kámo, ty ho neznáš? Býval velitelem stráží ve Starém táboře.
	AI_Output(self,other,"DIA_Addon_Ramon_FirstWarn_NO_07_02");	//Je to muž, kteréhu každý věří a respektuje ho.
...
{
	AI_Output(self,other,"DIA_Addon_Ramon_SecondWarn_07_00");	//Ještě o krok blíž a bude to tvůj poslední.
	other.aivar[AIV_LastDistToWP] = Npc_GetDistToWP(other,BDT_1071_Checkpoint);
...
func void DIA_Addon_Ramon_News_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Ramon_News_07_01");	//Nezkoušej to na mě. Pouze Franco rozhoduje o tom, kdo vstoupí do tábora.
};
...
func void DIA_Addon_Ramon_Lie_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Ramon_Lie_07_01");	//On ne. Tak buď vypadneš nebo ti zpřelámu kosti ...
	AI_StopProcessInfos(self);
};
...
{
	AI_Output(other,self,"DIA_Addon_Ramon_Perm_15_00");	//A? Vše v pořádku?
	AI_Output(self,other,"DIA_Addon_Ramon_Perm_07_01");	//Vše v pořádku - pokud vím, nikdo nemá povoleno jít dovnitř.
	AI_StopProcessInfos(self);
};
