...
instance DIA_Lester_SEND_XARDAS(C_Info)
{
	...
	permanent = FALSE;
	description = "Musíš říct Xardasovi o tom stínu, mohlo by to být důležité.";
};
...
func void DIA_Lester_SEND_XARDAS_Info()
{
	...
	AI_Output(self,other,"DIA_Lester_SEND_XARDAS_13_05");	//(povzdychne si) No dobrá, když je to tak důležité, tak za ním půjdu - ale ještě ne.
...
instance DIA_Addon_Lester_STADT(C_Info)
{
	...
	description = "Jsem na cestě do Khorinisu! Co o tom městě víš?";
};
...
{
	AI_Output(other,self,"DIA_Addon_Lester_STADT_15_00");	//Jsem na cestě do Khorinisu! Co o tom městě víš?
	AI_Output(self,other,"DIA_Addon_Lester_STADT_13_01");	//Khorinis? Hm, je to přístav. Nic zvláštního.
	AI_Output(self,other,"DIA_Addon_Lester_STADT_13_02");	//Proč se ptáš?
	AI_Output(other,self,"DIA_Addon_Lester_STADT_15_03");	//Potřebuju si promluvit s paladiny, kteří by měli být ve městě.
	AI_Output(self,other,"DIA_Addon_Lester_STADT_13_04");	//(směje se) To myslíš vážně? I kdyby ses už dostal do města, tak k paladinům nikdy.
};
...
instance DIA_Addon_Lester_Vorschlag(C_Info)
{
	...
	description = "Máš nějaký návrh, jak bych se mohl dostat do města?";
};
...
{
	AI_Output(other,self,"DIA_Addon_Lester_Vorschlag_15_00");	//Máš nějaký návrh, jak bych se mohl dostat do města?
	AI_Output(self,other,"DIA_Addon_Lester_Vorschlag_13_01");	//Vlastně ano. Nedávno jsem pracoval pro starého alchymistu Constantina.
	AI_Output(self,other,"DIA_Addon_Lester_Vorschlag_13_02");	//Má ve městě určitý vliv a nařídil strážím, aby pustili lidi, kteří mu nesou vzácné byliny.
	AI_Output(self,other,"DIA_Addon_Lester_Vorschlag_13_03");	//Ve skutečnosti to však není tak složité. Jednoduše nasbíráš hrst bylin, které tu rostou všude kolem. A pak jednoduše předstíráš, že děláš pro Constantina. Měli by tě pustit.
	AI_Output(self,other,"DIA_Addon_Lester_Vorschlag_13_04");	//Nicméně, ujisti se, že máš všechny druhy stejné. Ne jenom náhodně sebrané. Stráže nejsou příliš bystří a neví skoro nic o alchymii.
	AI_Output(self,other,"DIA_Addon_Lester_Vorschlag_13_05");	//Takže, trs, který obsahuje rostliny pouze jednoho druhu by je měl zmást.
	...
	B_LogEntry(TOPIC_Addon_PickForConstantino,"Lester si myslí, že bych se mohl dostat do města, když strážím u brány ukážu 10 rostlin stejného druhu. Jen musím předstírat, že je nesu pro alchymistu Constantina.");
...
func void DIA_Addon_Lester_PASSAGEPLANTSSUCCESS_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Lester_PASSAGEPLANTSSUCCESS_13_01");	//Jak říkám. Víš, že poslouchat moje rady se vyplatí, kámo.
...
func void DIA_Lester_PERM3_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Lester_PERM3_13_14");	//Ta bolest hlavy začíná být nesnesitelná. A teď na mě ještě pořád útočí ti ještěrani. Musím se sám sebe ptát, odkud se berou.
	}
	else
	{
