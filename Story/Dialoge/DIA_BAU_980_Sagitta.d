...
func void DIA_Sagitta_HALLO_was()
{
	...
	AI_Output(self,other,"DIA_Sagitta_HALLO_was_17_02");	//Les je mým přítelem. Poskytujeme mi vše, co potřebuju.
};
...
func void DIA_Sagitta_TeachAlchemyRequest_wo()
{
	...
	AI_Output(self,other,"DIA_Sagitta_TeachAlchemyRequest_wo_17_03");	//To je ten důvod, proč je pro mě tak obtížné tu kytku získat.
...
instance DIA_SAGITTA_TEACH2(C_Info)
{
	npc = BAU_980_Sagitta;
	nr = 91;
	condition = dia_sagitta_teach2_condition;
	information = dia_sagitta_teach2_info;
	permanent = 0;
	description = "Můžeš mě naučit něco o vaření?";
};


func int dia_sagitta_teach2_condition()
{
	return TRUE;
};

func void dia_sagitta_teach2_info()
{
	AI_Output(other,self,"DIA_Thekla_teach_15_00");	//Můžeš mě naučit něco o vaření?
	AI_Output(self,other,"DIA_Thekla_teach_01_01");	//Naučím tě svůj recept na Kompot.
	Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
	B_LogEntry(TOPIC_COOK,"Bylinkářka Sagitta mě může naučit vařit kompot.");
};


instance DIA_SAGITTA_LEARN2(C_Info)
{
	npc = BAU_980_Sagitta;
	nr = 92;
	condition = dia_sagitta_learn2_condition;
	information = dia_sagitta_learn2_info;
	permanent = 10;
	description = "Kompot z lučních bobulí (cena: 10 VB)";
};


func int dia_sagitta_learn2_condition()
{
	if(Npc_KnowsInfo(hero,dia_sagitta_teach2) && (KNOWS_COOKHPII == FALSE))
	{
		return TRUE;
	};
};

func void dia_sagitta_learn2_info()
{
	AI_Output(other,self,"DIA_Sagitta_Learn_15_00");	//Nauč mě vařit Kompot z lučních bobulí.
	if(hero.lp >= 10)
	{
		hero.lp = hero.lp - 10;
		AI_Output(self,other,"DIA_Sagitta_Learn_12_01");	//Tak a tet ti ukážu jak to uvaříš, sleduj pozorně.
		Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
		B_LogEntry(TOPIC_COOK,"Umím uvařit: Kompot z lučních bobulí.");
		B_LogEntry(TOPIC_COOK,"Ingredience jsou: 20 kusů lučních bobulí.");
		KNOWS_COOKHPII = TRUE;
		PrintScreen("Naučil ses uvařit: Kompot z lučních bobulí",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
...
func void DIA_Sagitta_HEAL_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Sagitta_HEAL_17_02");	//V týhle chvíli nepotřebuješ žádné léčení.
	};
};
