
instance DIA_Niclas_EXIT(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 999;
	condition = DIA_Niclas_EXIT_Condition;
	information = DIA_Niclas_EXIT_Info;
	permanent = TRUE;
	description = Dialog_Ende;
};


func int DIA_Niclas_EXIT_Condition()
{
	return TRUE;
};

func void DIA_Niclas_EXIT_Info()
{
	AI_StopProcessInfos(self);
};


instance DIA_Niclas_Hello(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 1;
	condition = DIA_Niclas_Hello_Condition;
	information = DIA_Niclas_Hello_Info;
	permanent = FALSE;
	important = TRUE;
};


func int DIA_Niclas_Hello_Condition()
{
	return TRUE;
};

func void DIA_Niclas_Hello_Info()
{
	AI_Output(self,other,"DIA_Niclas_Hello_03_00");	//Taky jsi na útěku?
	AI_Output(other,self,"DIA_Niclas_Hello_15_01");	//Proč si to myslíš?
	AI_Output(self,other,"DIA_Niclas_Hello_03_02");	//Proč bys sem jinak chodil tak sám? Tady není vůbec NIC.
};


instance DIA_Niclas_HaveALook(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 1;
	condition = DIA_Niclas_HaveALook_Condition;
	information = DIA_Niclas_HaveALook_Info;
	permanent = FALSE;
	description = "Jen jsem se tu chtěl trochu porozhlédnout po okolí.";
};


func int DIA_Niclas_HaveALook_Condition()
{
	return TRUE;
};

func void DIA_Niclas_HaveALook_Info()
{
	AI_Output(other,self,"DIA_Niclas_HaveALook_15_00");	//Jen jsem se tu chtěl trochu porozhlédnout po okolí.
	AI_Output(self,other,"DIA_Niclas_HaveALook_03_01");	//Dobrá, považuj se za mého hosta. Dal by sis kousek masa?
	CreateInvItems(self,ItFoMutton,1);
	Info_ClearChoices(DIA_Niclas_HaveALook);
	Info_AddChoice(DIA_Niclas_HaveALook,"Ne, díky.",DIA_Niclas_HaveALook_No);
	Info_AddChoice(DIA_Niclas_HaveALook,"S potěšením.",DIA_Niclas_HaveALook_Yes);
};

func void DIA_Niclas_HaveALook_No()
{
	AI_Output(other,self,"DIA_Niclas_HaveALook_No_15_00");	//Ne, díky.
	AI_Output(self,other,"DIA_Niclas_HaveALook_No_03_01");	//Jak je libo. Doufám, že ti nebude vadit, když si sám něco zakousnu.
	B_UseItem(self,ItFoMutton);
	Info_ClearChoices(DIA_Niclas_HaveALook);
};

func void DIA_Niclas_HaveALook_Yes()
{
	AI_Output(other,self,"DIA_Niclas_HaveALook_Yes_15_00");	//S potěšením.
	AI_Output(self,other,"DIA_Niclas_HaveALook_Yes_03_01");	//Tady, ta nejlepší krysokrtí kejtička. Podle mého vlastního receptu.
	B_GiveInvItems(self,other,ItFoMutton,1);
	Info_ClearChoices(DIA_Niclas_HaveALook);
};


instance DIA_Niclas_WhyHere(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 2;
	condition = DIA_Niclas_WhyHere_Condition;
	information = DIA_Niclas_WhyHere_Info;
	permanent = FALSE;
	description = "Tak proč jsi tady?";
};


func int DIA_Niclas_WhyHere_Condition()
{
	return TRUE;
};

func void DIA_Niclas_WhyHere_Info()
{
	AI_Output(other,self,"DIA_Niclas_WhyHere_15_00");	//Tak proč jsi tady?
	AI_Output(self,other,"DIA_Niclas_WhyHere_03_01");	//Nalezl jsem tu klid. Ve městě jsem se vždycky cítil tak stísněný.
};


instance DIA_Niclas_ShouldntWork(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 2;
	condition = DIA_Niclas_ShouldntWork_Condition;
	information = DIA_Niclas_ShouldntWork_Info;
	permanent = FALSE;
	description = "Neměl bys pracovat?";
};


func int DIA_Niclas_ShouldntWork_Condition()
{
	if(Npc_KnowsInfo(other,DIA_Niclas_WhyHere))
	{
		return TRUE;
	};
};

func void DIA_Niclas_ShouldntWork_Info()
{
	AI_Output(other,self,"DIA_Niclas_ShouldntWork_15_00");	//Neměl bys pracovat?
	AI_Output(self,other,"DIA_Niclas_ShouldntWork_03_01");	//Všechno, co potřebuju, si můžu udělat sám. Vyrábím si šípy a příroda mi poskytuje maso.
	AI_Output(other,self,"DIA_Niclas_ShouldntWork_15_02");	//Takže jsi lovec.
	AI_Output(self,other,"DIA_Niclas_ShouldntWork_03_03");	//Ano, pokud tomu tak chceš říkat. Ale lovím jen pro vlastní potřebu. Nelovím pro peníze.
};


instance DIA_Niclas_CanTeachMe(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 2;
	condition = DIA_Niclas_CanTeachMe_Condition;
	information = DIA_Niclas_CanTeachMe_Info;
	permanent = FALSE;
	description = "V tom případě bys mě mohl něco naučit.";
};


func int DIA_Niclas_CanTeachMe_Condition()
{
	if(Npc_KnowsInfo(other,DIA_Niclas_ShouldntWork))
	{
		return TRUE;
	};
};

func void DIA_Niclas_CanTeachMe_Info()
{
	AI_Output(other,self,"DIA_Niclas_CanTeachMe_15_00");	//V tom případě bys mě mohl něco naučit.
	AI_Output(self,other,"DIA_Niclas_CanTeachMe_03_01");	//Proč ne. Můžu ti ukázat, jak se zachází s lukem a výrobu střelné munice.
	Log_CreateTopic(TOPIC_Teacher,LOG_NOTE);
	B_LogEntry(TOPIC_Teacher,"Niclas mi může předvést, jak se zdokonalit v lukostřelbě.");
	B_LogEntry(TOPIC_Teacher,"Niclas mě může naučit vyrobit šípi a střeli.");
};


instance DIA_Niclas_Teach(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 2;
	condition = DIA_Niclas_Teach_Condition;
	information = DIA_Niclas_Teach_Info;
	permanent = TRUE;
	description = "Ukaž mi, jak se zachází s lukem.";
};


func int DIA_Niclas_Teach_Condition()
{
	if(Npc_KnowsInfo(other,DIA_Niclas_CanTeachMe))
	{
		return TRUE;
	};
};

func void DIA_Niclas_Teach_Info()
{
	AI_Output(other,self,"DIA_Niclas_Teach_15_00");	//Ukaž mi, jak se zachází s lukem.
	if(other.HitChance[NPC_TALENT_BOW] >= 60)
	{
		AI_Output(self,other,"DIA_Niclas_Teach_03_01");	//Už jsem tě naučil všemu, co znám. Měl by sis najít jiného učitele.
	}
	else
	{
		Info_ClearChoices(DIA_Niclas_Teach);
		Info_AddChoice(DIA_Niclas_Teach,Dialog_Back,DIA_Niclas_Teach_Back);
		Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow1,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1)),DIA_Niclas_Teach_BOW_1);
		Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow5,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1) * 5),DIA_Niclas_Teach_BOW_5);
	};
};

func void DIA_Niclas_Teach_Back()
{
	if(other.HitChance[NPC_TALENT_BOW] >= 60)
	{
		AI_Output(self,other,"DIA_Niclas_Teach_03_00");	//Už tě nemůžu naučit ničemu dalšímu.
	};
	Info_ClearChoices(DIA_Niclas_Teach);
};

func void DIA_Niclas_Teach_BOW_1()
{
	B_TeachFightTalentPercent(self,other,NPC_TALENT_BOW,1,60);
	Info_ClearChoices(DIA_Niclas_Teach);
	Info_AddChoice(DIA_Niclas_Teach,Dialog_Back,DIA_Niclas_Teach_Back);
	Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow1,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1)),DIA_Niclas_Teach_BOW_1);
	Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow5,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1) * 5),DIA_Niclas_Teach_BOW_5);
};

func void DIA_Niclas_Teach_BOW_5()
{
	B_TeachFightTalentPercent(self,other,NPC_TALENT_BOW,5,60);
	Info_ClearChoices(DIA_Niclas_Teach);
	Info_AddChoice(DIA_Niclas_Teach,Dialog_Back,DIA_Niclas_Teach_Back);
	Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow1,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1)),DIA_Niclas_Teach_BOW_1);
	Info_AddChoice(DIA_Niclas_Teach,B_BuildLearnString(PRINT_LearnBow5,B_GetLearnCostTalent(other,NPC_TALENT_BOW,1) * 5),DIA_Niclas_Teach_BOW_5);
};


instance DIA_NICLAS_LEARNII(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 3;
	condition = dia_niclas_learnii_condition;
	information = dia_niclas_learnii_info;
	permanent = TRUE;
	description = "Výroba šípu a střel (cena: 3 VB)";
};


func int dia_niclas_learnii_condition()
{
	if(Npc_KnowsInfo(hero,DIA_Niclas_CanTeachMe) && (KNOWS_ARROWBOLT == FALSE))
	{
		return TRUE;
	};
};

func void dia_niclas_learnii_info()
{
	AI_Output(other,self,"DIA_Niclas_Learn1_15_00");	//Nauč mě vyrobit šípi a střeli.
	if(hero.lp >= 3)
	{
		hero.lp = hero.lp - 3;
		B_TeachThiefTalent(self,other,NPC_TALENT_ARROW_BOLT);
		AI_Output(self,other,"DIA_Niclas_Learn1_12_01");	//Budeš potřebovat dřevo na výrobu a ocel.
		AI_Output(self,other,"DIA_Niclas_Learn1_12_02");	//Nejlépe se stím pracuje na truhlářské dílně.
		AI_Output(self,other,"DIA_Niclas_Learn1_12_04");	//A je to, už nebudeš muset platit takový prachy za střelivo.
		Log_CreateTopic(TOPIC_ARROW,LOG_NOTE);
		B_LogEntry(TOPIC_ARROW,"Umim vyrobit: šípi a střeli.");
		B_LogEntry(TOPIC_ARROW,"Material na výrobu: Dřevo na výrobu a surová ocel.");
		KNOWS_ARROWBOLT = TRUE;
		PrintScreen("Umím vyrobit: střelnou munici",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
		PrintScreen("Nedostatek vědomostních bodů!",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
		B_Say(self,other,"$NOLEARNNOPOINTS");
	};
};


instance DIA_Niclas_PICKPOCKET(C_Info)
{
	npc = BAU_984_Niclas;
	nr = 900;
	condition = DIA_Niclas_PICKPOCKET_Condition;
	information = DIA_Niclas_PICKPOCKET_Info;
	permanent = TRUE;
	description = Pickpocket_40;
};


func int DIA_Niclas_PICKPOCKET_Condition()
{
	return C_Beklauen(27,20);
};

func void DIA_Niclas_PICKPOCKET_Info()
{
	Info_ClearChoices(DIA_Niclas_PICKPOCKET);
	Info_AddChoice(DIA_Niclas_PICKPOCKET,Dialog_Back,DIA_Niclas_PICKPOCKET_BACK);
	Info_AddChoice(DIA_Niclas_PICKPOCKET,DIALOG_PICKPOCKET,DIA_Niclas_PICKPOCKET_DoIt);
};

func void DIA_Niclas_PICKPOCKET_DoIt()
{
	B_Beklauen();
	Info_ClearChoices(DIA_Niclas_PICKPOCKET);
};

func void DIA_Niclas_PICKPOCKET_BACK()
{
	Info_ClearChoices(DIA_Niclas_PICKPOCKET);
};

