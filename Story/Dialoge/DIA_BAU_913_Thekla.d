...
func void DIA_Thekla_PERM_Info()
{
	...
	if(MIS_Thekla_Paket == FALSE)
	{
		...
		AI_Output(self,other,"DIA_Thekla_PERM_17_08");	//Zajdi za Sagittou, léčitelkou, co žije za Sekobovou farmou, a přines mi od ní balík bylinek.
...
instance DIA_THEKLA_TEACH(C_Info)
{
	npc = BAU_913_Thekla;
	nr = 7;
	condition = dia_thekla_teach_condition;
	information = dia_thekla_teach_info;
	permanent = 0;
	description = "Můžeš mě naučit něco o vaření?";
};


func int dia_thekla_teach_condition()
{
	return TRUE;
};

func void dia_thekla_teach_info()
{
	...
	B_LogEntry(TOPIC_COOK,"Thekla na onarově farmě mě může naučit vařit kompot.");
};


instance DIA_THEKLA_LEARN(C_Info)
{
	npc = BAU_913_Thekla;
	nr = 8;
	condition = dia_thekla_learn_condition;
	information = dia_thekla_learn_info;
	permanent = 5;
	description = "Kompot z lesních bobulí (cena: 5 VB)";
};


func int dia_thekla_learn_condition()
{
	if(Npc_KnowsInfo(hero,dia_thekla_teach) && (KNOWS_COOKHPI == FALSE))
	{
		return TRUE;
	};
};

func void dia_thekla_learn_info()
{
	AI_Output(other,self,"DIA_thekla_Learn_15_00");	//Nauč mě vařit Kompot z lesních bobulí.
	if(hero.lp >= 5)
	{
		hero.lp = hero.lp - 5;
		AI_Output(self,other,"DIA_thekla_Learn_12_01");	//Tak a tet ti ukážu jak to uvaříš, sleduj pozorně.
		Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
		B_LogEntry(TOPIC_COOK,"Umím uvařit: Kompot z lesních bobulí.");
		B_LogEntry(TOPIC_COOK,"Ingredience jsou: 20 kusů lesních bobulí.");
		KNOWS_COOKHPI = TRUE;
		PrintScreen("Naučil ses uvařit: Kompot z lesních bobulí",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
