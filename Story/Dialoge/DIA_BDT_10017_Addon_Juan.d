...
{
	AI_Output(other,self,"DIA_Addon_Juan_HI_15_00");	//Nějaký blafy?
	AI_Output(self,other,"DIA_Addon_Juan_HI_13_01");	//Co otravuješ? Chceš mi něco říct? Jestli ne, tak si dej odchod.
	if(!Npc_IsDead(Freund))
	{
		AI_Output(self,other,"DIA_Addon_Juan_HI_13_02");	//Moji kamádi by si tě mohli hodit do parády, tak co ode mě chceš.
...
func void DIA_Addon_Juan_Losung_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Juan_Losung_13_01");	//Co? Esteban na mě něco zkouší? Mám plno práce a kůži sedřenou, jsem pořád v dole a makám, slyšíš MAKÁM.
	AI_Output(self,other,"DIA_Addon_Juan_Losung_13_02");	//...a on někoho pošle pro materiál? Já nejsem skladiště!
	AI_Output(other,self,"DIA_Addon_Juan_Losung_15_03");	//No? Tak to není můj problém.
	AI_Output(self,other,"DIA_Addon_Juan_Losung_13_04");	//UDĚLÁM z toho TVŮJ problém. Chceš tedy zásilku - ok, můžeš si ji koupit.
	AI_Output(self,other,"DIA_Addon_Juan_Losung_13_05");	//Vem si zlato od Estbana nebo Ravena nebo jinýho, je mi to jedno, ale kdo něco chce, ten si za to zaplatí.
};
