...
func void DIA_Torlof_Probe_Info()
{
	...
	if(Npc_KnowsInfo(other,DIA_Lee_OtherSld))
	{
		...
		AI_Output(self,other,"DIA_Torlof_Probe_01_03");	//(vzdychne si) Fajn. Tak dobrá. Než se k nám budeš moci přidat, musíš udělat dvě věci.
		...
		B_LogEntry(TOPIC_BecomeSLD,"Mám-li být přijat mezi žoldnéře, musím projít Torlofovou zkouškou a získat si úctu ostatních.");
	}
	else
	{
...
instance DIA_Torlof_Respekt(C_Info)
{
	...
	permanent = FALSE;
	description = "Jak si vysloužím respekt ostatních žoldnéřů?";
};
...
func void DIA_Torlof_RUF_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Torlof_RUF_01_13");	//Sentenza pro tebe ruku zvedne. Říká, že jsi se zachoval velice rozumně.
		Points_Sld = Points_Sld + 1;
	...
	if(Points_Sld >= 9)
	{
		...
		B_LogEntry(TOPIC_BecomeSLD,"Mezi žoldnéři už jsem si vydobyl dost úcty. Nyní bych měl prohodit pár slov s Leem.");
	}
...
func void DIA_Torlof_DEMENTOREN_Info()
{
	...
	Wld_InsertNpc(CastlemineDMT,"NW_CASTLEMINE_TOWER_06");
...
{
	AI_Output(other,self,"DIA_Torlof_BEMYCAPTAIN2_15_00");	//Brána do hradu v Hornickém údolí je otevřená a zaseknutá. Skřetům už nic nebrání v tom, aby nevzali hrad útokem.
...
func void DIA_Torlof_BEMYCAPTAIN3_ok()
{
	...
	if(B_GiveInvItems(other,self,ItMi_Gold,2500))
	{
		...
		B_LogEntry(Topic_Captain,"Jakmile jsem mu předal 2500 zlaťáků, je Torlof konečně připraven mě dopravit k cíli.");
	}
	else
	{
