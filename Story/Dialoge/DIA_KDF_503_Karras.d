...
func void DIA_Karras_Success_Info()
{
	...
	AI_Output(self,other,"DIA_Karras_Success_10_02");	//Ale teď je na čase, aby ses věnoval svým studiím.
...
{
	if(((hero.guild == GIL_NOV) && GIL_KDF) || (hero.guild == GIL_KDF))
	{
		return TRUE;
	};
};
...
func void DIA_Karras_KAP3_Perm_Info()
{
	...
	if((MIS_KarrasResearchDMT == FALSE) && (PyrokarToldKarrasToResearchDMT == TRUE) && (hero.guild == GIL_KDF))
	{
		...
		AI_Output(self,other,"DIA_Karras_KAP3_Perm_10_07");	//Ale kdyby se ti podařilo získat z nich něco, co bych mohl podrobit zkoumání...
	...
	else if(MIS_KarrasResearchDMT == LOG_Running)
	{
		...
		AI_Output(self,other,"DIA_Karras_KAP3_Perm_10_09");	//Samozřejmě, že jakmile něco zjistím, dám ti okamžitě vědět.
	}
	else
	{
		AI_Output(self,other,"DIA_Karras_KAP3_Perm_10_10");	//Nepřítel má mnoho tváří. Je strašně obtížné určit, která z nich představuje pro klášter největší nebezpečí.
	};
};
...
func void DIA_Karras_KAP3_Perm_Material()
{
	...
	Info_AddChoice(DIA_Karras_KAP3_Perm,"Takže chceš, abych ti přinesl mrtvolu?",DIA_Karras_KAP3_Perm_Corpse);
};

func void DIA_Karras_KAP3_Perm_WillSee()
{
	AI_Output(other,self,"DIA_Karras_KAP3_Perm_WillSee_15_00");	//Pokusím se.
	AI_Output(self,other,"DIA_Karras_KAP3_Perm_WillSee_10_01");	//To by bylo opravdu výborné. Mezitím se budu věnovat mému vlastnímu výzkumu.
	Info_ClearChoices(DIA_Karras_KAP3_Perm);
};
...
instance DIA_Karras_HaveBook(C_Info)
{
	...
	permanent = FALSE;
	description = "Mám u sebe Almanach posedlých. Možná by se ti mohl k něčemu hodit.";
};
...
func void DIA_Karras_ResearchDMTEnd_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Karras_ResearchDMTEnd_10_01");	//Ano. Už vím, kdo nebo co Pátrači skutečně jsou zač.
...
func void DIA_Karras_KarrasBlessedStone_Info()
{
	...
	B_LogEntry(TOPIC_DEMENTOREN,"Karras mi dal amulet, který mě ochrání před mentálními útoky Pátračů. Teď už bude všechno mnohem jednodušší.");
