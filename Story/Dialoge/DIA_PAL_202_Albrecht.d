...
func void DIA_Albrecht_TEACHPalRunes_Heal()
{
	...
	else if((PLAYER_TALENT_RUNES[SPL_PalFullHeal] == FALSE) && (Kapitel >= 5) && (MIS_KillOrkOberst == LOG_SUCCESS))
	{
		Info_ClearChoices(DIA_Albrecht_TEACHPalRunes);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,Dialog_Back,DIA_Albrecht_TEACHPalRunes_BACK);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,B_BuildLearnString("Léčba těžkých zranění",CostForPAlSpells),dia_albrecht_teachpalrunes_palfullheal);
	}
	else if(PLAYER_TALENT_RUNES[SPL_PalFullHeal] == TRUE)
	{
		AI_Output(self,other,"DIA_Albrecht_TEACHPalRunes_03_06");	//Už tě nemohu v tom dál učit, umíš vše co znám.
	}
	else
	{
...
func void DIA_Albrecht_TEACHPalRunes_Combat()
{
	...
	if((PLAYER_TALENT_RUNES[SPL_PalHolyBolt] == FALSE) && (MIS_ReadyforChapter4 == TRUE))
	{
		Info_ClearChoices(DIA_Albrecht_TEACHPalRunes);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,Dialog_Back,DIA_Albrecht_TEACHPalRunes_BACK);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,B_BuildLearnString("Svatá střela",CostForPAlSpells),DIA_Albrecht_TEACHPalRunes_PalHolyBolt);
	}
	...
	else if((PLAYER_TALENT_RUNES[SPL_PalDestroyEvil] == FALSE) && (Kapitel >= 5) && (MIS_KillOrkOberst == LOG_SUCCESS))
	{
		Info_ClearChoices(DIA_Albrecht_TEACHPalRunes);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,Dialog_Back,DIA_Albrecht_TEACHPalRunes_BACK);
		Info_AddChoice(DIA_Albrecht_TEACHPalRunes,B_BuildLearnString("Zničení zla",CostForPAlSpells),dia_albrecht_teachpalrunes_paldestroyevil);
	}
	else if(PLAYER_TALENT_RUNES[SPL_PalDestroyEvil] == TRUE)
	{
		AI_Output(self,other,"DIA_Albrecht_TEACHPalRunes_03_07");	//Už tě nemohu v tom dál učit, umíš vše co znám.
	}
	else
	{
		B_Albrecht_YouAreNotWorthy();
