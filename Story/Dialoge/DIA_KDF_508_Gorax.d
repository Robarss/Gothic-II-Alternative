...
instance DIA_Gorax_PICKPOCKET(C_Info)
{
	...
	permanent = TRUE;
	description = "(Ukrast tento klíč bude zatraceně těžké)";
};
...
instance DIA_Addon_Gorax_DaronsStatue(C_Info)
{
	...
	permanent = TRUE;
	description = "Mám tady sošku, kterou měl Daron doručit do kláštera.";
};
...
{
	AI_Output(other,self,"DIA_Addon_Gorax_DaronsStatue_15_00");	//Mám tady sošku, kterou měl Daron doručit do kláštera.
	if(Npc_HasItems(other,ItMi_LostInnosStatue_Daron))
	{
		AI_Output(self,other,"DIA_Addon_Gorax_DaronsStatue_14_01");	//(povzdychne) Je to hanebné, že takový vzácný kus sem musí přinést novic.
		AI_Output(self,other,"DIA_Addon_Gorax_DaronsStatue_14_02");	//A přece, je to svědectví o tvém odhodlaní sloužit Innosovi.
		if(B_GiveInvItems(other,self,ItMi_LostInnosStatue_Daron,1))
		{
			Npc_RemoveInvItems(self,ItMi_LostInnosStatue_Daron,1);
		};
		AI_Output(self,other,"DIA_Addon_Gorax_DaronsStatue_14_03");	//Buď si jistý mou vděčností, mladý novici.
	...
	{
		AI_Output(self,other,"DIA_Addon_Gorax_DaronsStatue_14_04");	//Soška? Nevidím žádnou sošku. Dones mi ji a já budu veřit tvému malému příbehu.
	};
};
...
instance DIA_Gorax_Wurst(C_Info)
{
	...
	permanent = TRUE;
	description = "Rozdělil jsem ty klobásy.";
};
...
func void DIA_Gorax_Aufgabe2_Info()
{
	...
	B_LogEntry(Topic_GoraxWein,"Mistr Gorax chce, abych hospodskému Orlanovi doručil dvanáct lahví vína. Cena za kus činí 240 zlaťáků.");
};
