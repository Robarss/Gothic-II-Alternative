...
func void DIA_Canthar_Hallo_Info()
{
	...
	if((Npc_HasEquippedArmor(other) == FALSE) || Hlp_IsItem(itm,itar_pchero))
	{
...
func void DIA_Canthar_WhatOffer_Info()
{
	...
	if(Canthar_GotMe == TRUE)
	{
		...
		if(Npc_HasEquippedArmor(other) == FALSE)
		{
			AI_Output(self,other,"DIA_Canthar_WhatOffer_09_05");	//S timhle kouskem papíru v kapse můžeš vypadat tak otrhaně, jak se ti zachce, a stráže tě nechají na pokoji.
		};
...
func void DIA_Canthar_Success_Info()
{
	...
	if(Canthar_Gefallen == FALSE)
	{
		AI_Output(other,self,"DIA_Canthar_Success_15_02");	//Chystal ses mi dát zbraň.
...
func void DIA_Canthar_Pay_Ja()
{
	...
	AI_Output(self,other,"DIA_Canthar_Pay_Ja_09_01");	//(směje se) Výborně. Tak to bych se měl podívat na to, aby tvá reputace mezi obchodníky zase stoupla.
