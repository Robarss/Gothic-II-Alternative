...
func void pc_met_info()
{
	if(Npc_HasItems(hero,ItFoMuttonRaw) >= 1)
	{
		...
		PrintScreen("Opekl jsi maso",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
...
instance PC_METIV(C_Info)
{
	npc = PC_Hero;
	condition = pc_metiv_condition;
	information = pc_metiv_info;
	permanent = 1;
	description = "Opéct 20x maso";
};


func int pc_metiv_condition()
{
	if((PLAYER_MOBSI_PRODUCTION == MOBSI_COOK2) && (MEATCOOKSTART == TRUE))
	{
		return TRUE;
...
func void pc_metiv_info()
{
	if(Npc_HasItems(hero,ItFoMuttonRaw) >= 20)
	{
		Npc_RemoveInvItems(hero,ItFoMuttonRaw,20);
		CreateInvItems(hero,ItFoMutton,20);
		PrintScreen("Opekl jsi maso",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
...
func void pc_metv_info()
{
	if(Npc_HasItems(hero,ItFo_Fish) >= 1)
	{
		...
		PrintScreen("Opekl jsi Rybu",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
		PrintScreen("Nedostatek ingrediencí!",-1,-1,"font_old_20_white.tga",1);
	...
	if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her)) || (Hlp_GetInstanceID(self) == Hlp_GetInstanceID(rock)))
	{
		if(Npc_HasItems(hero,ItFoMuttonRaw) >= 1)
		{
			...
			if(ilost_maso > 0)
			{
				...
				napis_maso = ConcatStrings(IntToString(ilost_maso)," Kusu masa opeèeno.");
				PrintScreen(napis_maso,-1,70,"font_old_20_white.tga",3);
			};
		}
		else
		{
