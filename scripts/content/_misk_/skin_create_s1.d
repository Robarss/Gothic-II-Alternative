...
instance PC_TRLBLARM(C_Info)
{
	...
	permanent = 1;
	description = "Zbroj z kùže èerného trola (2x kùže èerného trola, 3x Trolí tesák)";
};
...
func void pc_trlblarm_info()
{
	if((Npc_HasItems(hero,ItAt_TrollBlackFur) >= 2) && (Npc_HasItems(hero,ItAt_TrollTooth) >= 3))
	{
		...
		PrintScreen("Vyrobil jsi: Zbroj z kùže èerného trola!",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
...
instance PC_WOLFARM(C_Info)
{
	...
	permanent = 1;
	description = "Zbroj z kùže skøetího psa  (10x kùže skøetího psa, 10x drápy)";
};
...
func void pc_wolfarm_info()
{
	if((Npc_HasItems(hero,ItAt_WargFur) >= 10) && (Npc_HasItems(hero,ItAt_Claw) >= 10))
	{
		...
		PrintScreen("Vyrobil jsi: Zbroj z kuže skøetího psa!",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
		PrintScreen("Nemáš potøebný material!",-1,-1,"font_old_20_white.tga",1);
...
instance PC_SKINARM(C_Info)
{
	...
	permanent = 1;
	description = "Silná zbroj z kùže (5x vlèí kùže, 3x drápy)";
};
...
func void pc_skinarm_info()
{
	if((Npc_HasItems(hero,ItAt_WolfFur) >= 5) && (Npc_HasItems(hero,ItAt_Claw) >= 3))
	{
		...
		PrintScreen("Vyrobil jsi: Silnou zbroj z kùže!",-1,-1,"font_old_20_white.tga",1);
	}
	else
	{
		PrintScreen("Nemáš potøebný material!",-1,-1,"font_old_20_white.tga",1);
