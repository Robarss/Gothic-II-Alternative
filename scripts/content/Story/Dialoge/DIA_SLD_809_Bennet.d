...
func void DIA_Bennet_WannaSmith_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_WannaSmith_06_02");	//To tì bude nìco stát. Øeknìme 300 zlatıch.
	Info_ClearChoices(DIA_Bennet_WannaSmith);
	Info_AddChoice(DIA_Bennet_WannaSmith,"Moná pozdìji.",DIA_Bennet_WannaSmith_Later);
	Info_AddChoice(DIA_Bennet_WannaSmith,"Dobøe. Tady máš 300 zlatek.",DIA_Bennet_WannaSmith_Pay);
};

func void DIA_Bennet_WannaSmith_Pay()
{
	AI_Output(other,self,"DIA_Bennet_WannaSmith_Pay_15_00");	//Dobøe. Tady máš 300 zlatek.
	...
	{
		AI_Output(self,other,"DIA_Bennet_WannaSmith_Pay_06_02");	//Pøestaò se chovat jako naivka. Øíkám 300 a ani o minci míò.
	};
	Info_ClearChoices(DIA_Bennet_WannaSmith);
};
...
instance DIA_Bennet_DJG_ARMOR_M(C_Info)
{
	...
	permanent = TRUE;
	description = "Støednì tìká drakobijecká zbroj. Ochrana: zbranì 120, šípy 120. (6000 zlaákù).";
};
...
func void DIA_Bennet_BetterArmor_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_BetterArmor_06_07");	//Ale jen kvuli tobì si nebudu úètovat nic navíc, take to budeš mít levnìjší
};
...
{
	AI_Output(other,self,"DIA_Bennet_SellWhat_15_00");	//Mohl bys my odìlat helmu na zbroji drakobijcù?
	AI_Output(self,other,"DIA_Bennet_SellWhat_09_01");	//Ano kdy budeš mít zbroj tak není problém, pro drakobijce to je zdarma.
};
...
instance DIA_BENNET_LEATHER(C_Info)
{
	...
	permanent = TRUE;
	description = "(Zbroj drakobijcù bez ochrany hlavy)";
};
...
{
	AI_Output(other,self,"DIA_Bennet_LEATHER_15_00");	//Uprav my Tìkou
	...
	{
		AI_Output(self,other,"DIA_Bennet_LEATHER_09_02");	//Nemuu ti upravit zbroj kdy jí nemáš.
	};
	AI_Output(other,self,"DIA_Bennet_LEATHER_15_00");	//Uprav my Tìkou
...
func void dia_bennet_melting_info()
{
	...
	if(hero.lp >= 30)
	{
		...
		AI_Output(self,other,"DIA_Bennet_LEA_12_01");	//Nejdøív je nutné rozdìlat a oèistit skøetí nebo rezavé zbranì a potom teprve je hodíš do vıhnì.
		AI_Output(self,other,"DIA_Bennet_LEA_12_02");	//Ze tøí rozdìlanıch zbraní se dá udìlat jedna surová ocel a u skøetích to samé, ale musíš poèkat delší dobu ne se roztaví.
		AI_Output(self,other,"DIA_Bennet_LEA_12_04");	//A je to, nejlepší je surová elezná ruda z té se dá vyrobit mnohem víc surové ocele a to by bylo asi všechno.
		Log_CreateTopic(TOPIC_TalentSmith,LOG_NOTE);
		B_LogEntry(TOPIC_TalentSmith,"Bennet mì nauèil jak tavit skøetí a rezavé zbranì, nejprve je nutné je rozebrat a oèistit a tprve potom pøetavit na surovou ocel, na jednu ocel budu potøebovat 3x rozebrané staré rezavé elezo.");
