...
func void DIA_Cassia_Gilde_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Cassia_Gilde_16_00");	//Vidím, že ses pøidal na stranu práva a øádu.
		AI_Output(self,other,"DIA_Cassia_Gilde_16_01");	//Nevadí, že ses stal služebníkem Innose. Jsi jedním z nás. A já doufám, že se nìjaký zpùsob najde.
	};
	if(other.guild == GIL_KDF)
	{
		AI_Output(self,other,"DIA_Cassia_Gilde_16_02");	//Tak teï patøíš k Innosovì církvi. Fajn, ale ještì poøád jsi jedním z nás - doufám, že jsi na to nezapomnìl.
	};
	AI_StopProcessInfos(self);
};
...
func void DIA_Cassia_News_Info()
{
	if(MIS_ThiefGuild_sucked == FALSE)
	{
		...
		AI_Output(self,other,"DIA_Cassia_News_16_02");	//Upoutal jsi naši pozornost tím, že jsi dokázal získat dùvìru jednoho z našich pøátel.
		AI_Output(self,other,"DIA_Cassia_News_16_03");	//A my ti chceme dát šanci. Mùžeš se k nám pøidat.
	}
	else
	{
		...
		AI_Output(self,other,"DIA_Cassia_News_16_06");	//Chceme tì pøipravit o život za to, že jsi donášel na našeho pøítele. Proto jsme poslali Attilu.
		AI_Output(self,other,"DIA_Cassia_News_16_07");	//Nicménì, tvá pøítomnost tady nám poskytuje nové možnosti...
		AI_Output(other,self,"DIA_Cassia_News_15_08");	//... co mi chceš nabídnout?
		AI_Output(self,other,"DIA_Cassia_News_16_09");	//Mùžeš se k nám pøidat.
	};
...
func void DIA_Cassia_mehr_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_mehr_16_02");	//Ta hrstka lidí, co ví o existenci stok, vìøí, že jsou zamèené a nikdo se do nich nemùže dostat.
...
instance DIA_Cassia_MissingPeople(C_Info)
{
	...
	permanent = FALSE;
	description = "Je tu nìco, co mi mùžeš øíct o ztracených obyvatelích?";
};
...
{
	AI_Output(other,self,"DIA_Addon_Cassia_Add_15_00");	//Je tu nìco, co mi mùžeš øíct o ztracených obyvatelích?
	...
	AI_Output(self,other,"DIA_Addon_Cassia_Add_16_03");	//Když budeš opouštet stoku, plav mírnì doprava po pobreží.
...
{
	AI_Output(other,self,"DIA_Cassia_Vorteil_15_00");	//Co z toho budu mít, když se k vám pøidám?
	AI_Output(self,other,"DIA_Cassia_Vorteil_16_01");	//Mùžeš se od nás nauèit pár zajímavejch vìcí - schopnosti, které ti umožní žít v pøepychu.
...
func void DIA_Cassia_Lernen_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Lernen_16_01");	//Jesper je mistrem v plížení. Ukáže ti, jak se nehluènì pohybovat.
	AI_Output(self,other,"DIA_Cassia_Lernen_16_02");	//Ramirez je výjimeènì nadaný zlodìj. Jeho ruèièkám neodolá žádný zámek.
	AI_Output(self,other,"DIA_Cassia_Lernen_16_03");	//A já tì nauèím, jak si pøivlastòovat obsahy kapes. Cizích.
	AI_Output(self,other,"DIA_Cassia_Lernen_16_04");	//Taky ti pomùžu s tím, jak dosáhnout vìtší obratnosti. Protože obratnost je klíèem ke tvým schopnostem.
	Log_CreateTopic(TOPIC_CityTeacher,LOG_NOTE);
	B_LogEntry(TOPIC_CityTeacher,"Cassia mì nauèí kapsáøství a pomùže mi zvýšit obratnost.");
	B_LogEntry(TOPIC_CityTeacher,"Ramirez mì nauèí, jak páèit zámky.");
	B_LogEntry(TOPIC_CityTeacher,"Jesper mì nauèí plížení.");
};
...
func void DIA_Cassia_Regeln_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Regeln_16_05");	//A ètvrté, poslední pravidlo je: Každý, kdo se k nám chce pøidat, musí prokázat, že k nìèemu je.
};
...
instance DIA_Cassia_Erwischen(C_Info)
{
	...
	permanent = FALSE;
	description = "Co se stane, když mì chytnou?";
};
...
{
	AI_Output(other,self,"DIA_Cassia_Erwischen_15_00");	//Co se stane, když mì chytnou?
...
func void DIA_Cassia_beweisen_Info()
{
	...
	else
	{
		...
		AI_Output(self,other,"DIA_Cassia_beweisen_16_03");	//Ale jemu vážnì není k nièemu. Chci, aby zdobil mou ruku.
...
func void DIA_Cassia_Beitreten_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Beitreten_16_01");	//Výbornì. Dostal jsi šanci prokázat, co v tobì vìzí. A jestli se chceš od nás nìèemu pøiuèit, jsi tady vítán.
	Join_Thiefs = TRUE;
};
...
instance DIA_Cassia_Ablehnen(C_Info)
{
	...
	permanent = FALSE;
	description = "A co když se k vám nebudu chtít pøidat...?";
};
...
{
	AI_Output(other,self,"DIA_Cassia_Ablehnen_15_00");	//A co když se k vám nebudu chtít pøidat...?
	if(MIS_ThiefGuild_sucked == FALSE)
	{
		AI_Output(self,other,"DIA_Cassia_Ablehnen_16_01");	//Zahazuješ šanci, která se naskytne jen jednou, ale mùžeš jít.
		AI_Output(self,other,"DIA_Cassia_Ablehnen_16_02");	//(vážnì) A ani nepomysli na to, že bys nás nìkomu prozradil. Mohl bys toho trpce litovat.
...
func void DIA_Cassia_Ablehnen_Okay()
{
	...
	AI_Output(self,other,"DIA_Cassia_Ablehnen_Okay_16_01");	//(usmívá se) Uèinil jsi správné rozhodnutí. Jestli uspìješ ve zkoušce, mùžeš rozšíøit naše øady.
	AI_Output(self,other,"DIA_Cassia_Ablehnen_Okay_16_02");	//Jestli se chceš nejprve nauèit zlodìjské dovednosti, posluž si - budeš je potøebovat.
	Join_Thiefs = TRUE;
	Info_ClearChoices(DIA_Cassia_Ablehnen);
};
...
func void DIA_Cassia_Ablehnen_Kill()
{
	...
	AI_Output(self,other,"DIA_Cassia_Ablehnen_Kill_16_01");	//To je zlé. Mìla jsem dojem, že jsi chytøejší.
	AI_StopProcessInfos(self);
...
{
	AI_Output(other,self,"DIA_Cassia_BevorLernen_15_00");	//Mùžeš mì nìco nauèit?
	if(MIS_ThiefGuild_sucked == FALSE)
	{
		AI_Output(self,other,"DIA_Cassia_BevorLernen_16_01");	//Jasnì, žádný problém. Prostì mi dej vìdìt, až budeš pøipraven.
		Cassia_TeachPickpocket = TRUE;
		Cassia_TeachDEX = TRUE;
	}
	else
	{
		AI_Output(self,other,"DIA_Cassia_BevorLernen_16_02");	//Jasnì. Kapsáøství a obratnost tì bude každá stát 100 zlatých.
		Info_ClearChoices(DIA_Cassia_BevorLernen);
		Info_AddChoice(DIA_Cassia_BevorLernen,"Možná pozdìji... (ZPÌT)",DIA_Cassia_BevorLernen_Spaeter);
...
func void DIA_Cassia_BevorLernen_DEX()
{
	if(B_GiveInvItems(other,self,ItMi_Gold,100))
	{
		...
		AI_Output(self,other,"DIA_Cassia_BevorLernen_DEX_16_01");	//Mùžeme zaèít hned, jak budeš pøipraven.
		Cassia_TeachDEX = TRUE;
		Info_ClearChoices(DIA_Cassia_BevorLernen);
	}
	else
	{
		AI_Output(self,other,"DIA_Cassia_DIA_Cassia_BevorLernen_DEX_16_02");	//Vra se, až budeš mít zlato.
		Info_ClearChoices(DIA_Cassia_BevorLernen);
	};
};
...
func void DIA_Cassia_BevorLernen_Pickpocket()
{
	if(B_GiveInvItems(other,self,ItMi_Gold,100))
	{
		...
		AI_Output(self,other,"DIA_Cassia_BevorLernen_Pickpocket_16_01");	//Mùžeme zaèít hned, jak budeš pøipraven.
		Cassia_TeachPickpocket = TRUE;
		Info_ClearChoices(DIA_Cassia_BevorLernen);
	}
	else
	{
		AI_Output(self,other,"DIA_Cassia_BevorLernen_Pickpocket_16_02");	//Vra se, až budeš mít zlato.
		Info_ClearChoices(DIA_Cassia_BevorLernen);
	};
};
...
{
	AI_Output(other,self,"DIA_Cassia_TEACH_15_00");	//Chtìl bych být o nìco obratnìjší.
...
func void DIA_Cassia_Pickpocket_Info()
{
	...
	if(B_TeachThiefTalent(self,other,NPC_TALENT_PICKPOCKET))
	{
		...
		AI_Output(self,other,"DIA_Cassia_Pickpocket_16_02");	//Zatímco na nìj mluvíš, poøádnì si ho odhadni. Všímej si nacpaných kapes, šperkù nebo kožených øemínkù kolem krku. A hlavnì si hlídej, jak moc je ten chlápek všímavý.
		AI_Output(self,other,"DIA_Cassia_Pickpocket_16_03");	//Obrat nalitého nádeníka není to samé jako okrást ostražitého obchodníka, mìj to na pamìti.
		AI_Output(self,other,"DIA_Cassia_Pickpocket_16_04");	//Když budeš neohrabaný, tak samozøejmì bude èuchat zradu. Tak hlavnì klídek.
	};
};
...
func void DIA_Cassia_Aufnahme_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Aufnahme_16_02");	//Vezmi si tenhle klíè a nìco na sebe. Je od dveøí od hotelu. (úsmìv) Tak nebudeš muset pokaždé plavat.
	B_GiveInvItems(self,other,ItKe_ThiefGuildKey_Hotel_MIS,1);
	CreateInvItems(other,itar_noble_thief,1);
	AI_PrintScreen("Cassia ti daruje zlodìjský kabátec.",-1,YPOS_ItemTaken,FONT_ScreenSmall,2);
	AI_Output(self,other,"DIA_Cassia_Aufnahme_16_03");	//Taky bys mìl vìdìt, že máme tajné znamení. Zvláštní kývnutí.
	AI_PlayAni(other,"T_YES");
	AI_Output(self,other,"DIA_Cassia_Aufnahme_16_04");	//Pøesnì. Když budeš mluvit s tím pravým èlovìkem a udìláš tenhle posunek, bude vìdìt, že jsi jedním z nás.
	MIS_CassiaRing = LOG_SUCCESS;
	B_GivePlayerXP(XP_CassiaRing);
	Knows_SecretSign = TRUE;
	Log_CreateTopic(Topic_Diebesgilde,LOG_NOTE);
	B_LogEntry(Topic_Diebesgilde,"Byj jsem pøijat do zlodìjského cechu.");
	B_LogEntry(Topic_Diebesgilde,"Nauèil jsem se zlodìjský signál - když jej ukážu tìm správným lidem, poznají, že jsem jedním z nich.");
};
...
instance DIA_Cassia_Versteck(C_Info)
{
	...
	permanent = FALSE;
	description = "Tak kde ukrýváte svou koøist?";
};
...
{
	AI_Output(other,self,"DIA_Cassia_Versteck_15_00");	//Tak kde ukrýváte svou koøist?
	AI_Output(self,other,"DIA_Cassia_Versteck_16_01");	//Hele, snad si vážnì nemyslíš, že ti tohle øeknu?
	AI_Output(self,other,"DIA_Cassia_Versteck_16_02");	//Budeš mít dost pøíležitostí, aby sis užil své vlastní koøisti. Pamatuj - kdo je moc nenasytný, skonèí s prázdnýma rukama.
};
...
func void DIA_Cassia_Blutkelche_Info()
{
	...
	else
	{
		...
		AI_Output(self,other,"DIA_Cassia_Blutkelche_16_04");	//Král Rhobar je získal na jedné ze svých váleèných výprav -  ale obìtoval mnoho mužù na to, aby si je mohl odvézt. Proto se jim øíká Krvavé kalichy.
		AI_Output(self,other,"DIA_Cassia_Blutkelche_16_05");	//Žádný z tìch kalichù nemá sám o sobì velkou cenu - ale všech šest pohromadì pøedstavuje celé jmìní.
		AI_Output(other,self,"DIA_Cassia_Blutkelche_15_06");	//Kde jsou ty kalichy?
		AI_Output(self,other,"DIA_Cassia_Blutkelche_16_07");	//Jsou tady, ve mìstì - patøí bohatým obchodníkùm z horní ètvrti.
		...
		AI_Output(self,other,"DIA_Cassia_Blutkelche_16_10");	//Buï polovinu zisku, nebo si mùžeš vzít jeden zajímavý pøedmìt z mé koøisti.
		...
		B_LogEntry(Topic_CassiaKelche,"Cassia mì požádala, abych jí pøinesl šest krvavých kalichù. Nejspíš je všechny najdu kdesi ve mìstì.");
	};
};
...
func void DIA_Cassia_abgeben_Info()
{
	...
	if(B_GiveInvItems(other,self,ItMi_BloodCup_MIS,6))
	{
		...
		AI_Output(self,other,"DIA_Cassia_abgeben_16_03");	//Mùžeš hned vzít svou odmìnu. Dík, žes to pro mì udìlal.
		AI_Output(self,other,"DIA_Cassia_abgeben_16_04");	//Už pro tebe nemám nic na práci - ale mùžeš se ode mì kdykoli nìèemu nauèit. A navíc, na tomhle ostrovì je toho dost, co jen èeká, až si to nìkdo vezme. (usmívá se)
		MIS_CassiaKelche = LOG_SUCCESS;
		B_GivePlayerXP(XP_CassiaBlutkelche);
	}
	else
	{
		AI_Output(self,other,"DIA_Cassia_abgeben_16_05");	//Mùžu ty kalichy prodat jen jako sadu. Pøines mi je všechny.
	};
};
...
instance DIA_CASSIA_AKROBATI(C_Info)
{
	npc = VLK_447_Cassia;
	nr = 10;
	condition = dia_cassia_akrobati_condition;
	information = dia_cassia_akrobati_info;
	permanent = TRUE;
	description = "Ukaž mi, jak se lépe pohybovat (Akrobace). (10 LP, 500 zlatých, 60 obratonsti)";
};


func int dia_cassia_akrobati_condition()
{
	if((Npc_GetTalentSkill(other,NPC_TALENT_ACROBAT) == FALSE) && (MIS_CassiaRing == LOG_SUCCESS))
	{
		return TRUE;
	};
};

func void dia_cassia_akrobati_info()
{
	AI_Output(other,self,"DIA_Cassia_Akrobati_15_00");	//Ukaž mi, jak se lepe pohybovat.
	if((Npc_HasItems(other,ItMi_Gold) > 500) && (other.attribute[ATR_DEXTERITY] > 59) && (hero.lp >= 10))
	{
		Npc_RemoveInvItems(hero,ItMi_Gold,500);
		B_TeachThiefTalent(self,other,NPC_TALENT_ACROBAT);
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_01");	//Brzy získáš kontrolu nad svým tìlem a budeš schopen skákat mnohem dál.
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_02");	//Nauèil ses, jak se stoèit a jak nejlépe pøi pádu dopadnout na zem. Ale nezapomeò - nejsi nesmrtelný!.
		AI_Output(self,other,"DIA_Cassia_Akrobati_12_04");	//Akrobacie mùže být také velmi užiteèná v boji. Mùžeš velmi rychle mìnit vzdálenost mezi tebou a tvým nepøítelem. Hodnì štìstí!.
		PrintScreen("",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
		PrintScreen("Pøijd až budeš pøipraven!",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
