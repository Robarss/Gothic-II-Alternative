...
func void dia_thekla_teach_info()
{
	...
	B_LogEntry(TOPIC_COOK,"Thekla na onarovì farmì mì mùže nauèit vaøit kompot.");
};
...
{
	AI_Output(other,self,"DIA_thekla_Learn_15_00");	//Nauè mì vaøit Kompot z lesních bobulí.
	if(hero.lp >= 5)
	{
		hero.lp = hero.lp - 5;
		AI_Output(self,other,"DIA_thekla_Learn_12_01");	//Tak a tet ti ukážu jak to uvaøíš, sleduj pozornì.
		Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
		B_LogEntry(TOPIC_COOK,"Umím uvaøit: Kompot z lesních bobulí.");
		B_LogEntry(TOPIC_COOK,"Ingredience jsou: 20 kusù lesních bobulí.");
		KNOWS_COOKHPI = TRUE;
		PrintScreen("Nauèil ses uvaøit: Kompot z lesních bobulí",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
