...
func void DIA_Dragomir_Armbrust_Info()
{
	...
	AI_Output(self,other,"DIA_Dragomir_Armbrust11_12_04");	//A pokud budeš chtít mùu tì nauèit zakladní vırobu lukù.
	CreateInvItems(self,ItMi_Gold,150);
	B_GiveInvItems(self,other,ItMi_Gold,150);
	MIS_DragomirsArmbrust = LOG_SUCCESS;
	Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
	B_LogEntry(TOPIC_BOWHUNT,"Dragomir mì mùe nauèit základní vırobu lukù.");
...
instance DIA_DRAGOMIR_LEARNBOW1(C_Info)
{
	...
	permanent = TRUE;
	description = "Zakladní vıroba  (cena: 5 VB, 500 zlatıch)";
};
...
{
	AI_Output(other,self,"DIA_Dragomir_Learnbow1_15_00");	//Nauè mì zakladní vırobu.
	if((hero.lp >= 5) && (Npc_HasItems(other,ItMi_Gold) > 500))
	{
		...
		AI_Output(self,other,"DIA_Dragomir_Learnbow1_12_01");	//Zaèni tím, e si na dílnì na luky pøipravíš vìci na vırobu.
		AI_Output(self,other,"DIA_Dragomir_Learnbow1_12_02");	//Potom udìláš tohle a tam to, zbytek pochopíš z nákresu.
		AI_Output(self,other,"DIA_Dragomir_Learnbow1_12_04");	//Nákresy mùeš zakoupit u Bospra a lze ten samı postup pouít i na kuše.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
...
instance DIA_DRAGOMIR_BOWINFO(C_Info)
{
	...
	permanent = FALSE;
	description = "Nevíš kde bych se mohl nauèit pokroèilou a další vırobu lukù?";
};
...
{
	AI_Output(other,self,"DIA_Dragomir_Bowinfo_12_00");	//Nevíš kde bych se mohl nauèit pokroèilou a další vırobu lukù?
	AI_Output(self,other,"DIA_Dragomir_Bowinfo_12_01");	//Já umím jen základní a tu mì nauèil bosper ale ten uèí jen lovce.
	AI_Output(self,other,"DIA_Dragomir_Bowinfo_12_02");	//Zkus se poptat ostatních lovcù po ostrovì, tøeba ti vyhovı.
};
