...
func void dia_bosper_sellwhat_info()
{
	...
	AI_Output(self,other,"DIA_Bosper_SellWhat_09_01");	//Ano a to pøímo u mì, ale prodávám ho jen lovcùm, take pokud chceš tuhle loveckou zbroj musíš pro mì pracovat.
};
...
instance DIA_BOSPER_LEATHER(C_Info)
{
	...
	permanent = TRUE;
	description = "Koupit Zbroj Lovce. (1500 zlaákù)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_LEATHER_15_00");	//1500 zlaákù?? no dobøe, jen doufám e pøi lovu bude opravdu stát za to.
	if(B_GiveInvItems(other,self,ItMi_Gold,1500))
	{
		AI_Output(self,other,"DIA_Bosper_LEATHER_09_01");	//Bude, to se neboj a kdy budeš dobøe pracovat za chvíli to máš zpátky. (ušklíbne se)
	...
	{
		AI_Output(self,other,"DIA_Bosper_LEATHER_09_02");	//Zbroj má svou cenu - a opravdu za to stojí. Take se vra, a dáš dohromady dost zlata.
	};
};
...
func void dia_bosper_sellfur2_info()
{
	...
	if((Npc_HasItems(other,ItAt_Claw) > 0) || (Npc_HasItems(other,ItAt_LurkerClaw) > 0) || (Npc_HasItems(other,ItAt_Teeth) > 0) || (Npc_HasItems(other,ItAt_DrgSnapperHorn) > 0) || (Npc_HasItems(other,ItAt_ShadowHorn) > 0) || (Npc_HasItems(other,ItAt_SharkTeeth) > 0) || (Npc_HasItems(other,ItAt_TrollTooth) > 0))
	{
		...
		{
			AI_Output(self,other,"DIA_Bosper_SellFur1_10_03");	//Vıbornì, drápy èíhavce, ty jsou dobré.
		...
		{
			AI_Output(self,other,"DIA_Bosper_SellFur1_10_05");	//Páni, Roh draèího chòapavce, to jsou nechutné bestie.
		...
		{
			AI_Output(self,other,"DIA_Bosper_SellFur1_13_07");	//Zuby moèálového raloka, no tyhle bestie jsou taky nebezpeèné, tady máš zlato.
		...
		{
			AI_Output(self,other,"DIA_Bosper_SellFur1_14_08");	//A dokonce Trolí tesák, neuvìøitelné! to má cenu celého jmìní.
		...
		AI_Output(self,other,"DIA_Bosper_SellFur1_11_12");	//Dobrá práce. Pøijï zase, a seeneš další.
	}
	else
	{
		AI_Output(self,other,"DIA_Bosper_SellFur1_11_13");	//Ale vdy víš, e mì zajímají pouze rùzné kùe ze šelem, zuby, drápy a rohy, nic jiného.
	};
};
...
instance DIA_BOSPER_TEACH(C_Info)
{
	...
	permanent = 0;
	description = "Mùeš mì nauèit vyrobit zbroj?";
};
...
{
	AI_Output(other,self,"DIA_Bosper_teach_15_00");	//Mùeš mì nauèit vyrobit zbroj?
	AI_Output(self,other,"DIA_Bosper_teach_01_01");	//Mùu tì nauèit jak udìlat Silnìjší koenou zbroj.
	Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
	B_LogEntry(TOPIC_SKINHUN,"Bospr mì mùe nauèit jak vyrobit silnìjší koenou zbroj.");
};
...
instance DIA_BOSPER_LEARN(C_Info)
{
	...
	permanent = 2;
	description = "Silná koená zbroj (cena: 2 VB)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_Learn_15_00");	//Nauè mì vyrobit silnou koenou zbroj.
	if(hero.lp >= 2)
	{
		hero.lp = hero.lp - 2;
		AI_Output(self,other,"DIA_Bosper_Learn_12_01");	//Zaèni tím, e si na dílnì na kùe pøipravíš vìci na vırobu.
		AI_Output(self,other,"DIA_Bosper_Learn_12_02");	//Potom si pøiprav kùe ze kterıch budeš vyrábìt zbroj a oèisti ji.
		AI_Output(self,other,"DIA_Bosper_Learn_12_04");	//Natáhni je a sešívej pøesnì aby byla pevná tak jak ti to ukazuji.
		Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
		B_LogEntry(TOPIC_SKINHUN,"Umim vyrobit: Koenou zbroj.");
		B_LogEntry(TOPIC_SKINHUN,"Material na vırobu: 5x vlèí kùe, 3x drápy.");
		KNOWS_SKINARM = TRUE;
		PrintScreen("Umím vyrobit: Koenou zbroj",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
...
instance DIA_BOSPER_TEACHBOW(C_Info)
{
	...
	permanent = 0;
	description = "Mùeš mì nauèit jak na vırobu lukù?";
};
...
{
	AI_Output(other,self,"DIA_Bosper_bow_15_00");	//Mùeš mì nauèit jak na vırobu lukù?
	AI_Output(self,other,"DIA_Bosper_bow_01_01");	//Ano, mùu tì nauèit jak na vırobu lukù.
	Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
	B_LogEntry(TOPIC_BOWHUNT,"Bospr mì mùe nauèit jak vyrobit luky a kuše.");
};
...
instance DIA_BOSPER_LEARNBOW1(C_Info)
{
	...
	permanent = TRUE;
	description = "Zakladní vıroba  (cena: 2 VB)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_Learnbow1_15_00");	//Nauè mì zakladní vırobu.
	if(hero.lp >= 2)
	{
		hero.lp = hero.lp - 2;
		B_TeachThiefTalent(self,other,NPC_TALENT_BOW_MAKER);
		AI_Output(self,other,"DIA_Bosper_Learnbow1_12_01");	//Zaèni tím, e si na dílnì na luky pøipravíš vìci na vırobu.
		AI_Output(self,other,"DIA_Bosper_Learnbow1_12_02");	//Potom udìláš tohle a tam to, zbytek pochopíš z nákresu a dílna je pøímo za mnou.
		AI_Output(self,other,"DIA_Bosper_Learnbow1_12_04");	//Nákresy mùeš zakoupit i u mì a lze ten samı postup pouít i na kuše.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
...
instance DIA_BOSPER_LEARNBOW2(C_Info)
{
	...
	permanent = TRUE;
	description = "Pokroèilá vıroba  (cena: 4 VB)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_Learnbow2_15_00");	//Nauè mì pokroèilou vırobu.
	if(hero.lp >= 4)
	{
		hero.lp = hero.lp - 4;
		AI_Output(self,other,"DIA_Bosper_Learnbow2_12_01");	//Je to jen o trochu sloitìjší ale stojí to za to, podívej.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
		KNOWS_BOWSCROLLSII = TRUE;
		PrintScreen("Umím vyrobit pokroèilé luky a kuše",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
		PrintScreen("Nedostatek vìdomostních bodù!",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
...
instance DIA_BOSPER_LEARNBOW3(C_Info)
{
	...
	permanent = TRUE;
	description = "Mistrovská vıroba  (cena: 8 VB)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_Learnbow3_15_00");	//Nauè mì Mistrovskou vırobu.
	if(hero.lp >= 8)
	{
		hero.lp = hero.lp - 8;
		AI_Output(self,other,"DIA_Bosper_Learnbow3_12_01");	//Tady u to chce trochu umìní a trpìlivost, podívej.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
...
instance DIA_BOSPER_TEACHNOBOSPER(C_Info)
{
	...
	permanent = 0;
	description = "Mùeš mì nauèit Mistrovskou vırobu lukù?";
};
...
{
	AI_Output(other,self,"DIA_teachNoBosper_15_00");	//Mùeš mì nauèit Mistrovskou vırobu lukù?
	AI_Output(self,other,"DIA_teachNoBosper_15_01");	//Ne uèím jen lovce, mìl ses ke mì pøidat do uèení.
	AI_Output(other,self,"DIA_teachNoBosper_15_02");	//Nemohli bychom se nìjak dohodnout za rozumnou cenu?
	AI_Output(self,other,"DIA_teachNoBosper_15_03");	//Chceš se uèit mistrovskou úroveò? jene to není jen tak, rozumíš tomu aspoò trochu?
	AI_Output(other,self,"DIA_teachNoBosper_15_04");	//Ano ovládám u pokorèilou vırobu, tak co dohodnem se?
	AI_Output(self,other,"DIA_teachNoBosper_15_05");	//Hmmm.. v takovém pøípadì bych to mohl udìlat, ale nebude to levnı.
	AI_Output(other,self,"DIA_teachNoBosper_15_06");	//Kolik za to chceš?
	AI_Output(self,other,"DIA_teachNoBosper_15_07");	//3000 zlatıch a nehodlám smlouvat, take je to na tobì.
	Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
	B_LogEntry(TOPIC_BOWHUNT,"Bospr mì mùe nauèit Mistrovskou vırobu lukù.");
};
...
instance DIA_BOSPER_BOWNOBOSPER(C_Info)
{
	...
	permanent = TRUE;
	description = "Mistrovská vıroba  (cena: 15 VB, 3000 zlatıch)";
};
...
{
	AI_Output(other,self,"DIA_Bosper_BowNoBosper_15_00");	//Nauè mì Mistrovskou vırobu.
	if((hero.lp >= 15) && (Npc_HasItems(other,ItMi_Gold) > 3000))
	{
		hero.lp = hero.lp - 15;
		Npc_RemoveInvItems(hero,ItMi_Gold,3000);
		AI_Output(self,other,"DIA_Bosper_BowNoBosper_12_01");	//Tady u to chce trochu umìní a trpìlivost, podívej.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
		KNOWS_BOWSCROLLSIII = TRUE;
		PrintScreen("Umím vyrobit mistrovské luky a kuše",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
