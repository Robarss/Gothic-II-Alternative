...
{
	AI_Output(other,self,"DIA_Grom_teach_15_00");	//Mùžeš mì nauèit vyrobit zbroj?
	AI_Output(self,other,"DIA_Grom_teach_01_01");	//Mùžu tì nauèit jak udìlat zbroj z kùže skøetího psa.
	Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
	B_LogEntry(TOPIC_SKINHUN,"Grom mì mùže nauèi jak vyrobit zbroj z kùže skøetího psa.");
};
...
instance DIA_GROM_LEARN(C_Info)
{
	...
	permanent = 4;
	description = "Zbroj z kùže skøetího psa (cena: 4 VB)";
};
...
{
	AI_Output(other,self,"DIA_Grom_Learn_15_00");	//Nauè mì vyrobit zbroj z kùže skøetího psa.
	if(hero.lp >= 4)
	{
		hero.lp = hero.lp - 4;
		AI_Output(self,other,"DIA_Grom_Learn_12_01");	//Zaèni tím, že si na dílnì na kùže pøipravíš vìci na výrobu.
		AI_Output(self,other,"DIA_Grom_Learn_12_02");	//Potom si pøiprav kùže ze kterých budeš vyrábìt zbroj a oèisti ji.
		AI_Output(self,other,"DIA_Grom_Learn_12_04");	//Natáhni je a sešívej pøesnì aby byla pevná tak jak ti to ukazuji.
		Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
		B_LogEntry(TOPIC_SKINHUN,"Umim vyrobit: Zbroj z kùže skøetího psa.");
		B_LogEntry(TOPIC_SKINHUN,"Material na výrobu: 10x kùže skøetího psa, 10x drápy.");
		KNOWS_WOLFARM = TRUE;
		PrintScreen("Umím vyrobit: Zbroj z kùže skøetího psa.",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
