...
func void DIA_Dar_Hallo_Ja()
{
	...
	AI_Output(self,other,"DIA_Dar_Hallo_Ja_03_03");	//(k¯enÌ se) M·m svÈ zdroje.
	Info_ClearChoices(DIA_Dar_Hallo);
};
...
func void DIA_Dar_PERM_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Dar_PERM_03_01");	//(sarkasticky) NÏkdy se nech·m zml·tit od nÏkolika pomstychtiv˝ch feù·k˘ kv˘li t˝hle tr·vÏ...
		Dar_einmal = TRUE;
	}
	else
	{
...
instance DIA_Dar_DuDieb(C_Info)
{
	...
	permanent = FALSE;
	description = "Cipher mi ¯ekl, ûe mu nÏkdo ukradnul jeho balÌËek tr·vy.";
};
...
instance DIA_Dar_WoPaket(C_Info)
{
	...
	permanent = TRUE;
	description = "Kde je ten balÌËek?";
};
...
func void DIA_Dar_WoPaket_Info()
{
	...
	if(self.aivar[AIV_LastFightAgainstPlayer] == FIGHT_LOST)
	{
		...
		AI_Output(other,self,"DIA_Dar_WoPaket_15_03");	//Vypad· to, ûe pot¯ebujeö dalöÌ v˝prask?
		...
		AI_Output(self,other,"DIA_Dar_WoPaket_03_05");	//Bylo to v p¯Ìstavu blÌzko stavitel˘ lodÌ. To je vöechno, na co si vzpomÌn·m.
		Dar_Dieb = TRUE;
		B_LogEntry(Topic_CipherPaket,"Dar se p¯iznal, ûe balÌk s tr·vou ukradl on. Prodal jej v khorinisskÈ p¯ÌstavnÌ Ëtvrti, hned vedle loÔa¯ovy dÌlny.");
	}
	else
	{
...
{
	AI_Output(other,self,"DIA_Dar_Pilztabak_15_00");	//Uû jsi nÏkdy balil tab·k?
...
func void DIA_Dar_ORCRING_Info()
{
	...
	AI_Output(self,other,"DIA_Dar_ORCRING_03_01");	//JistÏ. To je pravda. Sylvio je na mÌle daleko a vzal p˘lku pos·dky s sebou.
	AI_Output(self,other,"DIA_Dar_ORCRING_03_02");	//To je jedno. M·m alespoÚ lepöÌ öanci st·t se jednÌm z Leeov˝ch chlapc˘. Vöechno, co pot¯ebuju, je se porvat.
...
func void DIA_Dar_ORCRING_necken()
{
	...
	if((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL))
	{
		AI_Output(self,other,"DIA_Dar_ORCRING_necken_03_05");	//Kdyû o tom tak p¯em˝ölÌm, rozbitÌ tvojÌ lebky je p¯esnÏ to, ËÌm bych si mohl u Leeov˝m chlapc˘ vyslouûit ten spr·vn˝ respekt.
		Info_ClearChoices(DIA_Dar_ORCRING);
		Info_AddChoice(DIA_Dar_ORCRING,"INem·m Ëas na tyhle kecy.",DIA_Dar_ORCRING_necken_no);
...
func void DIA_Dar_ORCRING_necken_schlagen()
{
	...
	AI_Output(self,other,"DIA_Dar_ORCRING_necken_schlagen_03_01");	//Ooch, nem˘ûu Ëekat.
	AI_StopProcessInfos(self);
...
func void DIA_Dar_ORCRING_wie()
{
	...
	AI_Output(self,other,"DIA_Dar_ORCRING_wie_03_02");	//T¯eba odznak od v˘dce sk¯et˘ nebo nÏco takovÈho. Prapor, prsten, nebo tak nÏco.
	AI_Output(self,other,"DIA_Dar_ORCRING_wie_03_03");	//S niËÌm menöÌm tady prorazit nejde. To je z¯ejmÈ.
...
func void DIA_Dar_FIGHTAGAINSTPALOVER_Info()
{
	...
	AI_Output(self,other,"DIA_Dar_FIGHTAGAINSTPALOVER_03_01");	//Nechci si tu nikoho znep¯·telit, tak na ten nesmysl prostÏ zapomeÚme, jo?
	B_GivePlayerXP(XP_Ambient);
	AI_StopProcessInfos(self);
};
...
func void DIA_Dar_BRINGORCELITERING_geld_no()
{
	...
	AI_Output(self,other,"DIA_Dar_BRINGORCELITERING_geld_no_03_01");	//A j· zase myslÌm, ûe je to nÏjak moc. Bez ur·ûky, tahle z·leûitost se mi zd· nÏjak podez¯el·.
	Info_ClearChoices(DIA_Dar_BRINGORCELITERING);
};
...
func void DIA_Dar_BRINGORCELITERING_was()
{
	...
	AI_Output(self,other,"DIA_Dar_BRINGORCELITERING_was_03_01");	//BuÔ si vezmi penÌze, nebo tenhle amulet, kter˝ jsem... no, ¯eknÏme ûe jsem jej p¯ed Ëasem nÏjak zÌskal.
