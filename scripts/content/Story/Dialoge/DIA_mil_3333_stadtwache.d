...
instance DIA_MIL_3333_STADTWACHE_BRIBE(C_Info)
{
	...
	permanent = TRUE;
	description = "Tady je 100 zlatých. Nech mì projít!";
};
...
{
	AI_Output(other,self,"DIA_Mil_310_Stadtwache_Bribe_15_00");	//Tady je 100 zlatých. Nech mì projít!
	if(B_GiveInvItems(other,self,ItMi_Gold,100))
	{
		AI_Output(self,other,"DIA_Mil_310_Stadtwache_Bribe_07_01");	//(hamižnì) 100 zlatých zní dobøe. Tak bìž.
		if(B_GetGreatestPetzCrime(self) >= CRIME_ATTACK)
		{
			AI_Output(self,other,"DIA_Mil_310_Stadtwache_Bribe_07_02");	//A jdi rovnou za lordem Andrem! Jinak tì to bude pøíštì stát dalších 100 zlatých!
		};
		self.aivar[AIV_PASSGATE] = TRUE;
...
func void dia_mil_3333_stadtwache_passierschein_info()
{
	...
	{
		AI_Output(self,other,"DIA_Mil_310_Stadtwache_Passierschein_07_01");	//Vážnì? A koho jsi kvùli ní zamordoval, ty vrahoune?
		AI_Output(other,self,"DIA_Mil_310_Stadtwache_Passierschein_15_02");	//Tak necháte mì projít, nebo ne?
		AI_Output(self,other,"DIA_Mil_310_Stadtwache_Passierschein_07_03");	//No, bìž dál, než si to rozmyslím!
	}
	else
	{
		AI_Output(self,other,"DIA_Mil_310_Stadtwache_Passierschein_07_04");	//Zdá se, že to je v poøádku. Tak bìž.
	};
	self.aivar[AIV_PASSGATE] = TRUE;
	Stadtwache_333.aivar[AIV_PASSGATE] = TRUE;
	Mil_310_schonmalreingelassen = TRUE;
