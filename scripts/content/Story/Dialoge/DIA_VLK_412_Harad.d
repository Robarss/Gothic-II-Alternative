...
func void dia_harad_melting_info()
{
	...
	if(hero.lp >= 10)
	{
		...
		AI_Output(self,other,"DIA_Harad_LEA_12_01");	//Nejdøív rozehøej výheò a potom vlož zbranì a nech je roztavit.
		AI_Output(self,other,"DIA_Harad_LEA_12_02");	//Z rezavé zbranì se dá udìlat jedna surová ocel a u skøetích to samé, ale musíš poèkat delší dobu než se roztavý.
		AI_Output(self,other,"DIA_Harad_LEA_12_04");	//A je to, nejlepší je surová železná ruda z té se dá vyrobit mnohem víc surové ocele ale to zjistíš sám.
...
func void dia_harad_newsmith_info()
{
	...
	if((hero.lp >= 5) && (Npc_HasItems(other,ItMi_Gold) > 1000))
	{
		...
		B_LogEntry(TOPIC_TalentSmith,"Harad mì nauèil kovat 'Spravedlnost' - nejlepší jednoruèní obyèejnou zbraò (Potøeba 5x rozžhavenou ocel).");
		KNOWS_NEWSMITH = TRUE;
...
func void dia_harad_newsmith2_info()
{
	...
	if((hero.lp >= 10) && (Npc_HasItems(other,ItMi_Gold) > 2000))
	{
		...
		AI_Output(self,other,"DIA_newsmith_12_01");	//Tady už to chce trochu umìní a trpìlivost, podívej.
		B_LogEntry(TOPIC_TalentSmith,"Harad mì nauèil kovat 'Mírotvùrce' - nejlepší obouruèní obyèejnou zbraò (Potøeba 10x rozžhavenou ocel).");
