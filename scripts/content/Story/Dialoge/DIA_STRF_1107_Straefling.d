...
{
	AI_Output(other,self,"DIA_STRF_1107_teach_15_00");	//Mùžeš mì nauèit nìco o vaøení?
	AI_Output(self,other,"DIA_STRF_1107_teach_01_01");	//Nauèím tì svùj recept na polévku z ústøic.
	Log_CreateTopic(TOPIC_COOKHU,LOG_NOTE);
	B_LogEntry(TOPIC_COOKHU,"Chef mì mùže nauèit vaøit polévku z ústøic.");
};
...
instance DIA_STRF_1107_LEARN(C_Info)
{
	...
	permanent = 5;
	description = "Specialní polévka z ústøic (cena: 5 VB)";
};
...
{
	AI_Output(other,self,"DIA_STRF_1107_Learn_15_00");	//Nauè mì vaøit Specialní polévku z ústøic.
	if(hero.lp >= 5)
	{
		hero.lp = hero.lp - 5;
		AI_Output(self,other,"DIA_Addon_Snaf_Learn_12_01");	//Tak a tet ti ukážu jak to uvaøíš, sleduj pozornì.
		Log_CreateTopic(TOPIC_COOKHU,LOG_NOTE);
		B_LogEntry(TOPIC_COOKHU,"Umím uvaøit: Specialní polévka z ústøic.");
		B_LogEntry(TOPIC_COOKHU,"Ingredience jsou: 2x Maso ze škeble, 10x Chòapavèí býlí, 1x Gobliní bobule.");
		KNOWS_COOKDEX = TRUE;
		PrintScreen("Nauèil ses uvaøit: Specialní polévka z ústøic",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
