...
instance DIA_XARDAS_SELLWHAT(C_Info)
{
	...
	permanent = FALSE;
	description = "Nemohl bys pro mì zjednodušit teleportaci?";
};
...
{
	AI_Output(other,self,"DIA_Xardas_SellWhat_15_00");	//Nemohl bys pro mì zjednodušit teleportaci?
	AI_Output(self,other,"DIA_Xardas_SellWhat_09_01");	//Asi vím co myslíš, vytvoøím rùnu která si bude pamatovat ostatní teleportaèní rùny, jen my pøines obyèejnou rùnu a máš to mít.
	AI_Output(self,other,"DIA_Xardas_SellWhat_09_02");	//Potom staèí jen použít novou rùnu a tvá rùna si to zapamatuje, takže už nebudeš muset hledat jiné rùnové kameny.
};
...
instance DIA_XARDAS_TELRUN(C_Info)
{
	...
	permanent = TRUE;
	description = "Vylepšit rùnovou telepotraci (1x rùnový kámen)";
};
...
{
	AI_Output(other,self,"DIA_Xardas_TELRUN_15_00");	//Tak tady je rùnoví kámen.
	if(B_GiveInvItems(other,self,ItMi_RuneBlank,1))
	{
		AI_Output(self,other,"DIA_Xardas_TELRUN_09_01");	//Hlavnì to nestra není to samoøejmost.
		CreateInvItems(other,ItRu_TeleportOWDemonTower,1);
		AI_PrintScreen("Teleportaèní rùna získána.",-1,YPOS_ItemTaken,FONT_ScreenSmall,2);
		XARDAS_LEATHERBOUGHT = TRUE;
	}
	else
	{
		AI_Output(self,other,"DIA_Xardas_TELRUN_09_02");	//Bez obyèejného rùnového kamene ti jí nevyrobím.
	};
};
