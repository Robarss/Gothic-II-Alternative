...
{
	AI_Output(other,self,"DIA_Edda_teach_15_00");	//Mùžeš mì nauèit nìco o vaøení?
	AI_Output(self,other,"DIA_Edda_teach_01_01");	//Nauèím tì svùj recept na pøípravu rybí polévky.
	Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
	B_LogEntry(TOPIC_COOK,"Edda mì mùže v pøístavní ètvrti nauèit vaøit.");
};
...
{
	AI_Output(other,self,"DIA_Edda_Learn_15_00");	//Nauè mì vaøit Rybí polévku.
	if(hero.lp >= 1)
	{
		hero.lp = hero.lp - 1;
		AI_Output(self,other,"DIA_Edda_Learn_12_01");	//Zaèni tím, že dáš vaøit vodu a vložíš do hrnce 2 ryby a zeleninu.
		AI_Output(self,other,"DIA_Edda_Learn_12_02");	//Jakmile se zaène vaøit, je dùležité aby jsi smìs poøád míchal a dochutil.
		AI_Output(self,other,"DIA_Edda_Learn_12_04");	//Až bude polévka hotová poznáš to díky jehí vùni.
		Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
		B_LogEntry(TOPIC_COOK,"Umím uvaøit: Rybí polévku.");
		B_LogEntry(TOPIC_COOK,"Ingredience jsou: 2 kusy Ryb.");
		KNOWS_FISHSOUP = TRUE;
		PrintScreen("Nauèil ses uvaøit: Rybí polévka",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
