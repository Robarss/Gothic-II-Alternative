...
{
	AI_Output(other,self,"DIA_Addon_Snaf_teach_15_00");	//Mùžeš mì nauèit nìco o vaøení?
	AI_Output(self,other,"DIA_Addon_Snaf_teach_01_01");	//Nauèím tì svùj recept na mùj specialní pokrm.
	Log_CreateTopic(TOPIC_COOKJ,LOG_NOTE);
	B_LogEntry(TOPIC_COOKJ,"Snaf mì mùže nauèit vaøit specialní pokrm.");
};
...
instance DIA_ADDON_SNAF_LEARN(C_Info)
{
	...
	permanent = 5;
	description = "Specialní pokrm ze žravých štìnic (cena: 5 VB)";
};
...
{
	AI_Output(other,self,"DIA_Addon_Snaf_Learn_15_00");	//Nauè mì vaøit Specialní pokrm ze žravých štìnic.
	if(hero.lp >= 5)
	{
		hero.lp = hero.lp - 5;
		AI_Output(self,other,"DIA_Addon_Snaf_Learn_12_01");	//Tak a tet ti ukážu jak to uvaøíš, sleduj pozornì.
		Log_CreateTopic(TOPIC_COOKJ,LOG_NOTE);
		B_LogEntry(TOPIC_COOKJ,"Umím uvaøit: Specialní pokrm ze žravých štìnic.");
		B_LogEntry(TOPIC_COOKJ,"Ingredience jsou: 3x žravé štìnice, 20x boruvky, 1x draèí koøen.");
		KNOWS_COOKSTR = TRUE;
		PrintScreen("Nauèil ses uvaøit: Specialní pokrm ze žravých štìnic",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
