...
func void dia_grimbald_sellwhat_info()
{
	...
	AI_Output(self,other,"DIA_Grimbald_SellWhat_09_01");	//No, tu jsem si vyrobil sám ale boužel už mám jen jednu s normálního trola, mùžu ti ji prodat za 1000, pochop tu cenu je s trollí kuže.
};
...
instance DIA_GRIMBALD_LEATHER(C_Info)
{
	...
	permanent = TRUE;
	description = "Koupit zbroj z trolla (1000 zlaákù)";
};
...
func void dia_grimbald_leather_info()
{
	...
	{
		AI_Output(self,other,"DIA_Grimbald_LEATHER_09_02");	//Odìv má svou cenu - a opravdu za to stojí. Takže se vra, až dáš dohromady dost zlata.
	};
};
...
func void DIA_Grimbald_Trolltot_Info()
{
	...
	AI_Output(self,other,"DIA_Grimbald_Trolltot1_07_00");	//Jsi opravdu rozený lovec, takže pokud rozumíš Zakladní výrobì lukù mùžu tì nauèit pokroèilu výrobu lukù.
	AI_Output(self,other,"DIA_Grimbald_Trolltot2_07_00");	//A jestli umíš stahovat kùže vím jak z èerného trolla vyrobit zbroj jako mám já.
	Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
	B_LogEntry(TOPIC_BOWHUNT,"Grimbald mì mùže nauèit Pokroèilou výrobu lukù.");
	B_GivePlayerXP(XP_Ambient);
};
...
instance DIA_GRIMBALD_TEACH(C_Info)
{
	...
	permanent = 0;
	description = "Mùžeš mì nauèit vyrobit zbroj?";
};
...
{
	AI_Output(other,self,"DIA_Grimbald_teach_15_00");	//Mùžeš mì nauèit vyrobit zbroj jakou máš ty?
	AI_Output(self,other,"DIA_Grimbald_teach_01_01");	//Potom co jsem vidìl jak jsi zabil toho trolla, tak není problém.
	Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
	B_LogEntry(TOPIC_SKINHUN,"Grimbald mì mùže nauèit jak vyrobit zbroj z kùže èerného trola.");
};
...
instance DIA_GRIMBALD_LEARN(C_Info)
{
	...
	permanent = 6;
	description = "Zbroj z kùže èerného trola (cena: 6 VB)";
};
...
{
	AI_Output(other,self,"DIA_Grimbald_Learn_15_00");	//Nauè mì vyrobit zbroj z kùže èerného trola.
	if(hero.lp >= 6)
	{
		hero.lp = hero.lp - 6;
		AI_Output(self,other,"DIA_Grimbald_Learn_12_01");	//Zaèni tím, že si na dílnì na kùže pøipravíš vìci na výrobu.
		AI_Output(self,other,"DIA_Grimbald_Learn_12_02");	//Potom si pøiprav kùže ze kterých budeš vyrábìt zbroj a oèisti ji.
		AI_Output(self,other,"DIA_Grimbald_Learn_12_04");	//Natáhni je a sešívej pøesnì aby byla pevná tak jak ti to ukazuji.
		Log_CreateTopic(TOPIC_SKINHUN,LOG_NOTE);
		B_LogEntry(TOPIC_SKINHUN,"Umim vyrobit: Zbroj z kùže èerného trola.");
		B_LogEntry(TOPIC_SKINHUN,"Material na výrobu: 2x kùže èerného trola, 3x Trolí tesák.");
		KNOWS_TRLBLARM = TRUE;
		PrintScreen("Umím vyrobit: Zbroj z kùže èerného trola",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
...
instance DIA_GRIMBALD_LEARNBOW2(C_Info)
{
	...
	permanent = TRUE;
	description = "Pokroèilá výroba  (cena: 10 VB, 1500 zlatých)";
};
...
{
	AI_Output(other,self,"DIA_grimbald_Learnbow2_15_00");	//Nauè mì pokroèilou výrobu.
	if((hero.lp >= 10) && (Npc_HasItems(other,ItMi_Gold) > 1500))
	{
		hero.lp = hero.lp - 10;
		Npc_RemoveInvItems(hero,ItMi_Gold,1500);
		AI_Output(self,other,"DIA_grimbald_Learnbow2_12_01");	//Zaèni tím, že si na dílnì na luky pøipravíš vìci na výrobu.
		AI_Output(self,other,"DIA_grimbald_Learnbow2_12_02");	//Potom udìláš tohle a tam to, zbytek pochopíš z nákresu.
		AI_Output(self,other,"DIA_grimbald_Learnbow2_12_04");	//Nákresy mùžeš zakoupit u Bospra a lze ten samý postup použít i na kuše.
		Log_CreateTopic(TOPIC_BOWHUNT,LOG_NOTE);
		B_LogEntry(TOPIC_BOWHUNT,"Umím vyrobit Pokroèilé luky a kuše.");
...
instance DIA_GRIMBALD_BOWINFO(C_Info)
{
	...
	permanent = TRUE;
	description = "Nevíš kde bych se mohl nauèit Mistrovskou výrobu lukù?";
};
...
{
	AI_Output(other,self,"DIA_grimbald_Bowinfo_12_00");	//Nevíš kde bych se mohl nauèit Mistrovskou výrobu lukù?
	AI_Output(self,other,"DIA_grimbald_Bowinfo_12_01");	//Líp než já to umí jen bosper ale ten uèí jen lovce, takže pokud pro nìj nepracuješ asi nemáš šanci.
	AI_Output(self,other,"DIA_grimbald_Bowinfo_12_02");	//Ale tøeba ho nìjak pøesvìdèíš.
};
