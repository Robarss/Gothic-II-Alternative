...
func void DIA_Niclas_CanTeachMe_Info()
{
	...
	AI_Output(self,other,"DIA_Niclas_CanTeachMe_03_01");	//Proè ne. Mùžu ti ukázat, jak se zachází s lukem a výrobu støelné munice.
	Log_CreateTopic(TOPIC_Teacher,LOG_NOTE);
	B_LogEntry(TOPIC_Teacher,"Niclas mi mùže pøedvést, jak se zdokonalit v lukostøelbì.");
	B_LogEntry(TOPIC_Teacher,"Niclas mì mùže nauèit vyrobit šípi a støeli.");
};
...
instance DIA_NICLAS_LEARNII(C_Info)
{
	...
	permanent = TRUE;
	description = "Výroba šípu a støel (cena: 3 VB)";
};
...
{
	AI_Output(other,self,"DIA_Niclas_Learn1_15_00");	//Nauè mì vyrobit šípi a støeli.
	if(hero.lp >= 3)
	{
		hero.lp = hero.lp - 3;
		B_TeachThiefTalent(self,other,NPC_TALENT_ARROW_BOLT);
		AI_Output(self,other,"DIA_Niclas_Learn1_12_01");	//Budeš potøebovat døevo na výrobu a ocel.
		AI_Output(self,other,"DIA_Niclas_Learn1_12_02");	//Nejlépe se stím pracuje na truhláøské dílnì.
		AI_Output(self,other,"DIA_Niclas_Learn1_12_04");	//A je to, už nebudeš muset platit takový prachy za støelivo.
		Log_CreateTopic(TOPIC_ARROW,LOG_NOTE);
		B_LogEntry(TOPIC_ARROW,"Umim vyrobit: šípi a støeli.");
		B_LogEntry(TOPIC_ARROW,"Material na výrobu: Døevo na výrobu a surová ocel.");
		KNOWS_ARROWBOLT = TRUE;
		PrintScreen("Umím vyrobit: støelnou munici",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
