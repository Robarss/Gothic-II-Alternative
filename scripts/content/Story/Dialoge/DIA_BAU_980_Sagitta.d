...
instance DIA_SAGITTA_TEACH2(C_Info)
{
	...
	permanent = 0;
	description = "Mùžeš mì nauèit nìco o vaøení?";
};
...
{
	AI_Output(other,self,"DIA_Thekla_teach_15_00");	//Mùžeš mì nauèit nìco o vaøení?
	AI_Output(self,other,"DIA_Thekla_teach_01_01");	//Nauèím tì svùj recept na Kompot.
	Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
	B_LogEntry(TOPIC_COOK,"Bylinkáøka Sagitta mì mùže nauèit vaøit kompot.");
};
...
instance DIA_SAGITTA_LEARN2(C_Info)
{
	...
	permanent = 10;
	description = "Kompot z luèních bobulí (cena: 10 VB)";
};
...
{
	AI_Output(other,self,"DIA_Sagitta_Learn_15_00");	//Nauè mì vaøit Kompot z luèních bobulí.
	if(hero.lp >= 10)
	{
		hero.lp = hero.lp - 10;
		AI_Output(self,other,"DIA_Sagitta_Learn_12_01");	//Tak a tet ti ukážu jak to uvaøíš, sleduj pozornì.
		Log_CreateTopic(TOPIC_COOK,LOG_NOTE);
		B_LogEntry(TOPIC_COOK,"Umím uvaøit: Kompot z luèních bobulí.");
		B_LogEntry(TOPIC_COOK,"Ingredience jsou: 20 kusù luèních bobulí.");
		KNOWS_COOKHPII = TRUE;
		PrintScreen("Nauèil ses uvaøit: Kompot z luèních bobulí",-1,-1,"FONT_OLD_20_WHITE.TGA",1);
	}
	else
	{
