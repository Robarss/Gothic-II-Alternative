...
{
	AI_Output(other,self,"DIA_Cedric_Teach_15_00");	//Zaènìme s výcvikem.
...
instance DIA_CEDRIC_TEACH2(C_Info)
{
	...
	permanent = TRUE;
	description = "(Boj ze štítem. Požadavek: 35% jednoruèních zbraní, cena: 5 VB)";
};
...
instance DIA_CEDRIC_TEACH3(C_Info)
{
	...
	permanent = TRUE;
	description = "(Pokroèilý boj ze štítem. Požadavek: 65% jednoruèních zbraní, cena: 10 VB)";
};
...
{
	AI_Output(other,self,"DIA_Babo_Teach_15_00");	//Zaènìme s tréninkem.
	if((other.lp >= 10) && (hero.aivar[REAL_TALENT_1H] >= 65))
	{
		other.lp = other.lp - 10;
		SCATTY_TEACH_PERM2 = TRUE;
		AI_UnequipWeapons(hero);
		PrintScreen("Nauèil ses: Pokroèilý boj se štítem",-1,-1,FONT_Screen,3);
	}
	else
	{
		B_Say(self,other,"$NOLEARNNOPOINTS");
...
instance DIA_CEDRIC_TEACH4(C_Info)
{
	...
	permanent = TRUE;
	description = "(Mistrovský boj ze štítem. Požadavek: 90% jednoruèních zbraní, cena: 15 VB)";
};
...
func void dia_cedric_teach4_info()
{
	...
	if((other.lp >= 15) && (hero.aivar[REAL_TALENT_1H] >= 90))
	{
		...
		AI_UnequipWeapons(hero);
		PrintScreen("Nauèil ses: Mistrovský boj se štítem",-1,-1,FONT_Screen,3);
	}
	else
	{
		B_Say(self,other,"$NOLEARNNOPOINTS");
