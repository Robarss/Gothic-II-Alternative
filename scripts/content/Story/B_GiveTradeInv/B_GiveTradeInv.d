...
func void B_GiveTradeInv(var C_Npc slf)
{
	...
	trd_alwin = Hlp_GetNpc(Vlk_424_Alwin);
	...
	trd_horas = Hlp_GetNpc(kdf_59_horas);
	trd_abuyin = Hlp_GetNpc(VLK_456_Abuyin);
	...
	if(slf.aivar[AIV_ChapterInv] <= Kapitel)
	{
		...
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_alwin))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_alwin(slf);
		};
		...
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_horas))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_horas(slf);
		};
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_abuyin))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_abuyin(slf);
		};
