...
func int C_TowerBanditsDead()
{
	...
	if(Npc_IsDead(TowerBandit1) || !Hlp_IsValidNpc(TowerBandit1))
	{
		deadcount += 1;
	};
	if(Npc_IsDead(TowerBandit2) || !Hlp_IsValidNpc(TowerBandit2))
	{
		deadcount += 1;
	};
	if(Npc_IsDead(TowerBandit3) || !Hlp_IsValidNpc(TowerBandit3))
	{
		deadcount += 1;
	};
	if(deadcount == 3)
