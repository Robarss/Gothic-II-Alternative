# Gothic-II-Alternative-Scripts (v1.2)

This mod contains the basic scripts from the mod https://www.moddb.com/mods/uscs-shield-mod/downloads/uscs-shield-eng
which I subsequently edited and remade into the Czech language of the original cz scripts.


Some scripts are implemented here from Gothic II L'hiver edition 1.6, Gothic I - Die Welt der Verurteilten, Gothic II Retruring 2.0

I would like to especially thank you for the help with creation and editing from Fafkes and from the translation part Auroren.

